export default {
    offerAccepted: {
        ru: 'Ваше предложение принято',
        en: 'Your offer has been accepted',
        ua: 'Вашу пропозицію прийнято',
        de: 'Dein Angebot wurde angenommen',
        pl: 'Twoja oferta została zaakceptowana',
        zh: '你方报盘已被接受',
    },

    offerDeclined: {
        ru: 'Ваше предложение отклонено',
        en: 'Your offer has been declined',
        ua: 'Вашу пропозицію відхилено',
        de: 'Dein Angebot wurde abgelehnt',
        pl: 'Twoja oferta została odrzucona',
        zh: '你方报盘已被拒绝',
    },

    inviteMember: {
        ru: 'Приглашает вас вступить в {{name}}',
        ua: 'Запрошує вас приєднатися до {{name}}',
        en: 'Invites you to join {{name}}',
        de: 'Lädt dich ein, dich {{name}} anzuschließen',
        pl: 'Zaprasza cię do przyłączenia się do {{name}}',
        zh: '邀请您加入{{name}}',
    },

    acquaintance: {
        ru: 'Предлагает познакомиться',
        ua: 'Пропонує познайомитися',
        en: 'Suggests getting acquainted',
        de: 'Schlägt eine Bekanntschaft vor',
        pl: 'Sugeruje zapoznanie się',
        zh: '建议大家互相了解',
    },

    passport: {
        ru: 'Предлагает посмотреть свой паспорт',
        ua: 'Пропонує переглянути свій паспорт',
        en: 'Offers to show their passport',
        de: 'Bietet an, den eigenen Pass zu zeigen',
        pl: 'Proponuje pokazać swój paszport',
        zh: '出示护照',
    },

    licenses: {
        ru: 'Предлагает посмотреть свои лицензии',
        ua: 'Запрошує переглянути свої ліцензії',
        en: 'Invites to view their licenses',
        de: 'Lädt dazu ein, die eigenen Lizenzen anzusehen',
        pl: 'Zaprasza do obejrzenia swoich licencji',
        zh: '查看他们的许可证',
    },

    //

    sellPills: {
        ru: 'Предлагает вам купить {{count}} шт. таблеток {{name}} за ${{sum}}',
        ua: 'Пропонує тобі купити {{count}} шт. таблеток {{name}} за ${{sum}}',
        en: 'Offers you to buy {{count}} pcs of {{name}} pills for ${{sum}}',
        de: 'Bietet dir {{count}} Stück {{name}}-Tabletten für ${{sum}} an',
        pl: 'Oferuje ci kupno {{count}} sztuk tabletek {{name}} za ${{sum}}',
    },

    giveMedicalLisense: {
        ru: 'Предлагает получить медецинскую лицензию за ${{sum}}',
        ua: 'Пропонує отримати медичну ліцензію за ${{sum}}',
        en: 'Offers to get a medical license for ${{sum}}',
        de: 'Bietet dir an, eine medizinische Lizenz für ${{sum}} auszustellen',
        pl: 'Oferuje uzyskanie licencji medycznej za ${{sum}}',
        zh: '提供医疗执照${{sum}}',
    },
    renewMedicalLicense: {
        ru: 'Предлагает продлить вашу медицинскую лицензию за ${{sum}}',
        ua: 'Пропонує продовжити вашу медичну ліцензію за ${{sum}}',
        en: 'Offers to renew your medical license for ${{sum}}',
        de: 'Bietet dir an, deine medizinische Lizenz für ${{sum}} zu verlängern',
        pl: 'Oferuje przedłużenie twojej licencji medycznej za ${{sum}}',
        zh: '提供更新您的医疗执照${{sum}}',
    },
    takeBlood: {
        ru: 'Предлагает взять у вас кровь за ${{sum}}',
        ua: 'Пропонує взяти у вас кров за ${{sum}}',
        en: 'Offers to take your blood for ${{sum}}',
        de: 'Bietet dir an, dir für ${{sum}} Blut abzunehmen',
        pl: 'Oferuje pobranie Twojej krwi za ${{sum}}',
        zh: '以${{sum}}的价格抽你的血',
    },
    healDiseases: {
        ru: '{{name}} вас вылечит за ${{sum}}',
        ua: '{{name}} вилікує вас за ${{sum}}',
        en: '{{name}} will heal you for ${{sum}}',
        de: '{{name}} wird dich für ${{sum}} heilen',
        pl: '{{name}} wyleczy cię za ${{sum}}',
    },

    //
    showBadge: {
        ru: 'Предлагает вам показать свое удостоверение',
        ua: 'Пропонує вам показати своє посвідчення',
        en: 'Offers you to show your badge',
        de: 'Bietet dir an, seinen Ausweis vorzuzeigen',
        pl: 'Proponuje ci pokazanie swojej odznaki',
    },

    drivingLicenses: {
        ru: 'Предлагает посмотреть своё водительское удостоверение',
        ua: 'Пропонує подивитися своє водійське посвідчення',
        en: "Offers to show their driver's license",
        de: 'Bietet an, seinen Führerschein zu zeigen',
        pl: 'Oferuje pokazanie swojego prawa jazdy',
    },

    tradeAssets: {
        ru: 'Предлагает обменяться имуществом',
        ua: 'Пропонує обмінятися майном',
        en: 'Offers to trade properties',
        de: 'Bietet einen Eigentumstausch an',
        pl: 'Proponuje wymianę majątku',
        zh: '提议进行财产交换',
    },
};
