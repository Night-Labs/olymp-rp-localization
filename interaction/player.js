export const player = {
    acquaintance: {
        ru: 'Познакомиться',
        ua: 'Познайомитися',
        en: 'Get Acquainted',
        de: 'Sich kennenlernen',
        pl: 'Poznaj się',
        zh: '见面',
    },
    giveMoney: {
        ru: 'Передать деньги',
        ua: 'Передати гроші',
        en: 'Give money',
        de: 'Geld geben',
        pl: 'Przekazać pieniądze',
        zh: '转账',
    },

    robPlayer: {
        ru: 'Ограбить',
        ua: 'Ограбувати',
        en: 'Rob',
        de: 'Rauben',
        pl: 'Ograbić',
        zh: '抓',
    },

    takeLicenses: {
        ru: 'Отобрать лицензии',
        ua: 'Відібрати ліцензії',
        en: 'Revoke Licenses',
        de: 'Lizenzen entziehen',
        pl: 'Odbierać licencje',
        zh: '吊销许可证',
    },

    giveBouquet: {
        ru: 'Подарить букет',
        ua: 'Подарувати букет',
        en: 'Give bouquet',
        de: 'Blumenstrauß schenken',
        pl: 'Podaruj bukiet',
        zh: '送花束',
    },
    giveBouquetHeader: {
        ru: 'Букет',
        ua: 'Букет',
        en: 'Bouquet',
        de: 'Blumenstrauß',
        pl: 'Bukiet',
        zh: '花束',
    },
    youGaveBouquet: {
        ru: 'Вы подарили букет {{name}}',
        ua: 'Ви подарували букет {{name}}',
        en: 'You gave a bouquet to {{name}}',
        de: 'Du hast {{name}} einen Blumenstrauß geschenkt',
        pl: 'Podarowałeś bukiet graczowi {{name}}',
        zh: '你送了一束花给{{name}}',
    },
    youGotBouquet: {
        ru: '{{name}} подарил(а) вам букет',
        ua: '{{name}} подарував(ла) вам букет',
        en: '{{name}} gave you a bouquet',
        de: '{{name}} hat dir einen Blumenstrauß geschenkt',
        pl: '{{name}} podarował(a) ci bukiet',
        zh: '{{name}}送了你一束花',
    },
    youDontHoldBouquet: {
        ru: 'У вас в руках нет букета',
        ua: 'У вас у руках немає букета',
        en: 'You are not holding a bouquet',
        de: 'Du hältst keinen Blumenstrauß in den Händen',
        pl: 'Nie trzymasz bukietu w rękach',
        zh: '你手里没有花束',
    },
    youHaveNoBouquet: {
        ru: 'У вас нет букета в инвентаре',
        ua: 'У вас немає букета в інвентарі',
        en: 'You do not have a bouquet in your inventory',
        de: 'Du hast keinen Blumenstrauß in deinem Inventar',
        pl: 'Nie masz bukietu w ekwipunku',
        zh: '你的背包里没有花束',
    },
    targetNoSpaceForBouquet: {
        ru: 'У {{name}} недостаточно места в инвентаре',
        ua: 'У {{name}} недостатньо місця в інвентарі',
        en: '{{name}} does not have enough inventory space',
        de: '{{name}} hat nicht genug Platz im Inventar',
        pl: '{{name}} nie ma wystarczająco miejsca w ekwipunku',
        zh: '{{name}}的背包空间不足',
    },
    failedToGiveBouquet: {
        ru: 'Не удалось подарить букет',
        ua: 'Не вдалося подарувати букет',
        en: 'Failed to give the bouquet',
        de: 'Der Blumenstrauß konnte nicht verschenkt werden',
        pl: 'Nie udało się podarować bukietu',
        zh: '无法赠送花束',
    },
    giveMoneyHeader: {
        ru: 'Передача денег',
        ua: 'Передача грошей',
        en: 'Money Transfer',
        de: 'Geldüberweisung',
        pl: 'Przekazywanie pieniędzy',
        zh: '转账',
    },

    youGaveMoney: {
        ru: 'Вы передали {{value}}$ {{name}}',
        ua: 'Ви передали {{value}}$ {{name}}',
        en: 'You gave {{value}}$ to {{name}}',
        de: 'Du hast {{value}}$ an {{name}} überwiesen',
        pl: 'Przekazałeś {{value}}$ dla {{name}}',
        zh: '你给了{{value}$给{{name}}',
    },

    youGotMoney: {
        ru: 'Вы получили {{value}}$ от {{name}}',
        ua: 'Ви отримали {{value}}$ від {{name}}',
        en: 'You received {{value}}$ from {{name}}',
        de: 'Du hast {{value}}$ von {{name}} erhalten',
        pl: 'Otrzymałeś {{value}}$ od {{name}}',
        zh: '你从{{name}}得到了{{value}}$',
    },

    // Faction
    faction: {
        ru: 'Фракционное',
        en: 'Faction',
        ua: 'Фракційне',
        de: 'Fraktion',
        pl: 'Frakcja',
        zh: '功能性',
    },

    inviteMember: {
        ru: 'Пригласить игрока',
        en: 'Invite player',
        ua: 'Запросити гравця',
        de: 'Spieler einladen',
        pl: 'Zaprosić gracza',
        zh: '邀请玩家',
    },

    kickMember: {
        ru: 'Выгнать игрока из фракции',
        en: 'Kick player from faction',
        ua: 'Вигнати гравця з фракції',
        de: 'Spieler aus der Fraktion werfen',
        pl: 'Wyrzucić gracza z frakcji',
        zh: '将玩家踢出阵营',
    },

    // ems

    reanimate: {
        ru: 'Реанимировать',
        ua: 'Реанімувати',
        en: 'Resuscitate',
        de: 'Wiederbeleben',
        pl: 'Reanimować',
        zh: '复活',
    },
    sellReliefix: {
        ru: 'Продать Reliefix',
        ua: 'Продати Reliefix',
        en: 'Sell Reliefix',
        de: 'Reliefix verkaufen',
        pl: 'Sprzedać Reliefix',
        zh: '出售救济品',
    },
    sellPainaway: {
        ru: 'Продать Painaway',
        ua: 'Продати Painaway',
        en: 'Sell Painaway',
        de: 'Painaway verkaufen',
        pl: 'Sprzedać Painaway',
        zh: '卖掉痛苦',
    },
    sellNufrieren: {
        ru: 'Продать Nufrieren',
        ua: 'Продати Nufrieren',
        en: 'Sell Nufrieren',
        de: 'Nufrieren verkaufen',
        pl: 'Sprzedaj Nufrieren',
    },

    takeBlood: {
        ru: 'Взять кровь',
        ua: 'Взяти кров',
        en: 'Take blood',
        de: 'Blut nehmen',
        pl: 'Pobrać krew',
        zh: '抽血',
    },
    giveLicense: {
        ru: 'Продать лицензию',
        ua: 'Продати ліцензію',
        en: 'Sell license',
        de: 'Lizenz verkaufen',
        pl: 'Sprzedaj licencję',
    },

    checkPlayerDiseases: {
        ru: 'Проверить здоровье игрока',
        ua: "Перевірити здоров'я гравця",
        en: "Check player's health",
        de: 'Spielergesundheit überprüfen',
        pl: 'Sprawdź zdrowie gracza',
    },
    healPlayerDiseases: {
        ru: 'Вылечить болезни игрока',
        ua: 'Вилікувати хвороби гравця',
        en: "Cure player's diseases",
        de: 'Spielerkrankheiten heilen',
        pl: 'Wylecz choroby gracza',
    },

    healPlayer: {
        ru: 'Вылечить игрока',
        ua: 'Вилікувати гравця',
        en: 'Heal the player',
        de: 'Spieler heilen',
        pl: 'Wylecz gracza',
    },

    //

    cuff: {
        ru: 'Надеть наручники',
        ua: 'Надіти кайданки',
        en: 'Cuff',
        de: 'Handschellen anlegen',
        pl: 'Założyć kajdanki',
        zh: '戴上手铐',
    },
    uncuff: {
        ru: 'Снять наручники',
        ua: 'Зняти кайданки',
        en: 'Uncuff',
        de: 'Handschellen abnehmen',
        pl: 'Zdjąć kajdanki',
        zh: '解开手铐',
    },
    follow: {
        ru: 'Вести за собой',
        ua: 'Вести за собою',
        en: 'Follow',
        de: 'Folgen',
        pl: 'Prowadzić',
        zh: '跟我来',
    },
    unfollow: {
        ru: 'Перестать вести за собой',
        ua: 'Перестати вести за собою',
        en: 'Unfollow',
        de: 'Nicht mehr folgen',
        pl: 'Przestać prowadzić',
        zh: '别跟着我',
    },

    //
    snowball: {
        ru: 'Слепить снежку',
        ua: 'Зліпити сніжок',
        en: 'Make a snowball',
        de: 'Einen Schneeball machen',
        pl: 'Ulepić śnieżkę',
        zh: '堆雪',
    },
    suicide: {
        ru: 'Суицид',
        ua: 'Суїцид',
        en: 'Suicide',
        de: 'Selbstmord',
        pl: 'Samobojstwo',
        zh: '自杀',
    },

    //
    passport: {
        ru: 'Посмотреть свой паспорт',
        ua: 'Переглянути свій паспорт',
        en: 'View your passport',
        de: 'Eigenen Pass ansehen',
        pl: 'Zobacz swój paszport',
        zh: '查看您的护照',
    },
    showPassport: {
        ru: 'Показать паспорт',
        ua: 'Показати паспорт',
        en: 'Show passport',
        de: 'Pass zeigen',
        pl: 'Pokaż paszport',
        zh: '出示护照',
    },

    showDrivingLicenses: {
        ru: 'Показать водительское удостоверение',
        ua: 'Показати водійське посвідчення',
        en: 'Show driving license',
        de: 'Führerschein anzeigen',
        pl: 'Pokaż prawo jazdy',
    },

    // frisk
    frisk: {
        ru: 'Обыск',
        ua: 'Обшук',
        en: 'Frisk',
        de: 'Abtasten',
        pl: 'Przeszukanie',
        zh: '搜索',
    },

    viewPassport: {
        ru: 'Посмотреть паспорт',
        ua: 'Переглянути паспорт',
        en: 'View passport',
        de: 'Pass ansehen',
        pl: 'Zobacz paszport',
        zh: '看看护照',
    },

    viewLicenses: {
        ru: 'Посмотреть лицензии',
        ua: 'Переглянути ліцензії',
        en: 'View licenses',
        de: 'Lizenzen ansehen',
        pl: 'Zobacz licencje',
        zh: '查看许可证',
    },

    viewDrivingLicenses: {
        ru: 'Посмотреть водительское удостоверение',
        ua: 'Переглянути водійське посвідчення',
        en: "View driver's license",
        de: 'Führerschein anzeigen',
        pl: 'Wyświetl prawo jazdy',
    },

    showLicenses: {
        ru: 'Показать лицензии',
        ua: 'Показати ліцензії',
        en: 'Show licenses',
        de: 'Lizenzen anzeigen',
        pl: 'Pokaż licencje',
        zh: '出示许可证',
    },

    dispatches: {
        ru: 'Вызовы',
        ua: 'Виклики',
        en: 'Dispatches',
        de: 'Notrufe',
        pl: 'Wysyłki',
        zh: '呼叫',
    },

    issueFine: {
        ru: 'Выписать штраф',
        ua: 'Виписати штраф',
        en: 'Issue Fine',
        de: 'Bußgeld verhängen',
        pl: 'Nakład grzywny',
        zh: '写罚款',
    },

    reviveAidKit: {
        ru: 'Реанимировать аптечкой',
        ua: 'Реанімувати аптечкою',
        en: 'Revive with aid kit',
        de: 'Mit Verbandskasten wiederbeleben',
        pl: 'Reanimować zestawem pierwszej pomocy',
        zh: '用急救箱恢复活力',
    },

    // trading
    trading: {
        ru: 'Торговля',
        ua: 'Торгівля',
        en: 'Trading',
        de: 'Handel',
        pl: 'Handel',
        zh: '贸易',
    },
    sellVehicle: {
        ru: 'Продать Т/С',
        ua: 'Продати Т/З',
        en: 'Sell Vehicle',
        de: 'Fahrzeug verkaufen',
        pl: 'Sprzedać pojazd',
        zh: '车辆销售',
    },
    sellHouse: {
        ru: 'Продать Дом',
        ua: 'Продати Будинок',
        en: 'Sell House',
        de: 'Haus verkaufen',
        pl: 'Sprzedać dom',
        zh: '卖掉房子',
    },

    // pairs animations
    pairsAnimations: {
        title: {
            ru: 'Парные анимации',
            ua: 'Парні анімації',
            en: 'Paired Animations',
            de: 'Interaktionen',
            pl: 'Animacje Par',
            zh: '配对动画',
        },
        shoulder: {
            ru: 'Положить на плечо',
            ua: 'Покласти на плече',
            en: 'Put on Shoulder',
            de: 'Auf die Schulter legen',
            pl: 'Położyć na ramię',
            zh: '扛在肩上',
        },
        carry: {
            ru: 'Нести на руках',
            ua: 'Нести на руках',
            en: 'Carry in Arms',
            de: 'Auf den Armen tragen',
            pl: 'Nieść na rękach',
            zh: '把它放在你的手上',
        },
        carryBehind: {
            ru: 'Нести на спине',
            ua: 'Нести на спині',
            en: 'Carry on Back',
            de: 'Auf dem Rücken tragen',
            pl: 'Nieść na plecach',
            zh: '背上',
        },
        takeHostage: {
            ru: 'Взять в заложники',
            ua: 'Взяти в заручники',
            en: 'Take Hostage',
            de: 'Als Geisel nehmen',
            pl: 'Wziąć zakładnika',
            zh: '劫持人质',
        },

        embrace: {
            ru: 'Обнять',
            ua: 'Обійняти',
            en: 'Embrace',
            de: 'Umarmen',
            pl: 'Objąć',
            zh: '拥抱',
        },
        kiss: {
            ru: 'Поцеловать',
            ua: 'Поцілувати',
            en: 'Kiss',
            de: 'Küssen',
            pl: 'Pocałować',
            zh: '吻',
        },
        five: {
            ru: 'Пять',
            ua: "П'ять",
            en: 'Five',
            de: 'Fünf',
            pl: 'Pięć',
            zh: '五个',
        },
        slap: {
            ru: 'Ударить по лицу',
            ua: 'Вдарити по обличчю',
            en: 'Slap',
            de: 'Schlagen ins Gesicht',
            pl: 'Uderzyć w twarz',
            zh: '打脸',
        },
    },

    // Party
    party: {
        ru: 'Группа',
        ua: 'Група',
        en: 'Group',
        de: 'Gruppe',
        pl: 'Grupa',
    },
    createParty: {
        ru: 'Создать группу',
        ua: 'Створити групу',
        en: 'Create group',
        de: 'Gruppe erstellen',
        pl: 'Utwórz grupę',
    },
    disbandParty: {
        ru: 'Распустить группу',
        ua: 'Розпустити групу',
        en: 'Disband group',
        de: 'Gruppe auflösen',
        pl: 'Rozwiąż grupę',
    },
    inviteToParty: {
        ru: 'Пригласить в группу',
        ua: 'Запросити до групи',
        en: 'Invite to group',
        de: 'Zur Gruppe einladen',
        pl: 'Zaproś do grupy',
    },
    leaveParty: {
        ru: 'Покинуть группу',
        ua: 'Покинути групу',
        en: 'Leave group',
        de: 'Gruppe verlassen',
        pl: 'Opuść grupę',
    },
    kickFromParty: {
        ru: 'Выгнать из группы',
        ua: 'Вигнати з групи',
        en: 'Kick from group',
        de: 'Aus der Gruppe entfernen',
        pl: 'Wyrzuć z grupy',
    },
    armyTicket: {
        ru: 'Выдать военный билет',
        ua: 'Видати військовий квиток',
        en: 'Issue military ID',
        de: 'Militärausweis ausstellen',
        pl: 'Wydać bilet wojskowy',
    },

    // doors
    doorOpen: {
        ru: 'Открыть дверь',
        ua: 'Відкрити двері',
        en: 'Open door',
        de: 'Tür öffnen',
        pl: 'Otwórz drzwi',
    },
    doorClose: {
        ru: 'Закрыть дверь',
        ua: 'Закрити двері',
        en: 'Close door',
        de: 'Tür schließen',
        pl: 'Zamknij drzwi',
    },

    // new
    unCabbleTie: {
        ru: 'Снять стяжки',
        ua: 'Зняти стяжки',
        en: 'Remove cable ties',
        de: 'Kabelbinder entfernen',
        pl: 'Usuń opaski kablowe',
    },
    cabbleTie: {
        ru: 'Надеть стяжки',
        ua: 'Надіти стяжки',
        en: 'Put on cable ties',
        de: 'Kabelbinder anlegen',
        pl: 'Załóż opaski kablowe',
    },
    takeMask: {
        ru: 'Снять маску',
        ua: 'Зняти маску',
        en: 'Take off the mask',
        de: 'Maske abnehmen',
        pl: 'Zdejmij maskę',
    },
    showBadge: {
        ru: 'Показать удостоверение',
        ua: 'Показати посвідчення',
        en: 'Show ID',
        de: 'Zeig den Ausweis',
        pl: 'Pokaż identyfikator',
    },

    //
    house: {
        ru: 'Дом',
        ua: 'Дім',
        en: 'House',
        de: 'Haus',
        pl: 'Dom',
    },
    'house-menu': {
        ru: 'Меню дома',
        ua: 'Меню дому',
        en: 'House menu',
        de: 'Hausmenü',
        pl: 'Menu domu',
    },
    'house-invite': {
        ru: 'Подселить',
        ua: 'Підселити',
        en: 'Invite to live together',
        de: 'Mitbewohner einladen',
        pl: 'Zaprosić do wspólnego mieszkania',
    },
    'house-kickPlayer': {
        ru: 'Выгнать игрока из дома',
        ua: 'Вигнати гравця з дому',
        en: 'Kick player out of the house',
        de: 'Spieler aus dem Haus werfen',
        pl: 'Wyrzucić gracza z domu',
    },
};
