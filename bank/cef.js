export const cef = {
    welcome: {
        title: {
            ua: 'Ласкаво просимо до',
            en: 'Welcome to',
            ru: 'Добро пожаловать в',
            de: 'Willkommen bei',
            pl: 'Witamy w',
            zh: '欢迎来到',
        },
        desc: {
            ua: 'Наш банк успішно обслуговує клієнтів вже десять років, надаючи їм якісні фінансові послуги. Ми пишаємося задоволеними клієнтами, які залишаються з нами протягом всього цього часу. Нашим пріоритетом є задоволеність наших клієнтів, і ми намагаємося забезпечити їм високий рівень обслуговування. Дякуємо всім за довіру, яку ви виявляєте нам вже довгі роки!',
            en: 'Our bank has been successfully serving customers for ten years, providing them with quality financial services. We take pride in satisfied customers who have stayed with us throughout this time. Our priority is customer satisfaction, and we strive to provide them with a high level of service. Thank you all for the trust you have shown us for many years!',
            ru: 'Наш банк успешно обслуживает клиентов уже десять лет, предоставляя им качественные финансовые услуги. Мы гордимся довольными клиентами, которые остаются с нами на протяжении всего этого времени. Наш приоритет – удовлетворенность наших клиентов, и мы стараемся обеспечить им высокий уровень обслуживания. Спасибо всем за доверие, которое вы оказываете нам уже долгие годы!',
            de: 'Unsere Bank bedient Kunden seit zehn Jahren erfolgreich und bietet ihnen qualitativ hochwertige Finanzdienstleistungen. Wir sind stolz auf zufriedene Kunden, die uns während dieser Zeit treu geblieben sind. Unsere Priorität ist die Kundenzufriedenheit, und wir bemühen uns, ihnen einen hohen Servicestandard zu bieten. Vielen Dank an alle für das Vertrauen, das du uns über viele Jahre entgegengebracht hast!',
            pl: 'Nasz bank od dziesięciu lat skutecznie obsługuje klientów, świadcząc dla nich usługi finansowe najwyższej jakości. Jesteśmy dumni z zadowolonych klientów, którzy pozostali z nami przez cały ten czas. Naszym priorytetem jest zadowolenie klientów, i dążymy do zapewnienia im wysokiego poziomu obsługi. Dziękujemy wszystkim za zaufanie, jakie nam wykazaliście przez wiele lat!',
            zh: '本行成立十年来，一直以优质的服务深受大家喜爱，感谢大家多年来对我们的信任!',
        },
        button: {
            ua: 'Відкрити рахунок',
            en: 'Open an Account',
            ru: 'Открыть счёт',
            de: 'Konto eröffnen',
            pl: 'Otwórz konto',
            zh: '开设账户',
        },
    },
    pincode: {
        create: {
            title: {
                ua: 'Ще трохи...',
                en: 'Just a Bit More...',
                ru: 'Ещё чуть чуть...',
                de: 'Noch ein bisschen...',
                pl: ' Jeszcze trochę...',
                zh: '只要给多一点...',
            },
            desc: {
                ua: 'Для завершення створення картки потрібно придумати та ввести пін-код',
                en: 'To complete the card creation, you need to come up with and enter a PIN code',
                ru: 'Для завершения создания карты нужно придумать и ввести пин-код',
                de: 'Um die Kartenerstellung abzuschließen, musst du eine PIN eingeben',
                pl: 'Aby zakończyć tworzenie karty, musisz wymyślić i wprowadzić kod PIN',
                zh: '注册银行卡，请设置密码',
            },
            button: {
                ua: 'Готово',
                en: 'Done',
                ru: 'Готово',
                de: 'Fertig',
                pl: 'Gotowe',
                zh: '完成',
            },
        },
        signin: {
            title: {
                ua: 'Пін-код',
                en: 'PIN Code',
                ru: 'Пин-код',
                de: 'PIN-Code',
                pl: 'Kod PIN',
                zh: '密码',
            },
            desc: {
                ua: 'Для входу в меню управління рахунком введіть ваш пін-код',
                en: 'To access the account management menu, enter your PIN code',
                ru: 'Чтобы войти в меню управления счётом введите ваш пин-код',
                de: 'Gib deinen PIN-Code ein, um auf das Kontoverwaltungsmenü zugreifen zu können',
                pl: 'Aby uzyskać dostęp do menu zarządzania kontem, wprowadź swój kod PIN',
                zh: '请输入密码',
            },
            button: {
                ua: 'Вхід',
                en: 'Sign In',
                ru: 'Войти',
                de: 'Anmelden',
                pl: 'Zaloguj się',
                zh: '登录',
            },
        },
    },
    buttons: {
        info: {
            name: {
                ru: 'Информация',
                en: 'Information',
                ua: 'Інформація',
                de: 'Information',
                pl: 'Informacja',
                zh: '资讯',
            },
        },
        deposit: {
            name: {
                ua: 'Внесення коштів',
                en: 'Deposit',
                ru: 'Внесение средств',
                de: 'Einzahlung',
                pl: 'Depozyt',
                zh: '资讯',
            },
            desc: {
                ua: 'На цій сторінці ви можете внести кошти на свій банківський рахунок',
                en: 'On this page, you can deposit funds into your bank account',
                ru: 'На этой странице вы можете внести средства на свой банковский счёт',
                de: 'Auf dieser Seite kannst du Geld auf dein Bankkonto einzahlen',
                pl: 'Na tej stronie możesz wpłacić środki na swoje konto bankowe',
                zh: '在这个页面上，你可以把钱存入你的银行账户',
            },
        },
        withdraw: {
            name: {
                ua: 'Виведення коштів',
                en: 'Withdraw',
                ru: 'Снятие средств',
                de: 'Abheben',
                pl: 'Wypłata',
                zh: '取钱',
            },
            desc: {
                ua: 'На цій сторінці ви можете зняти кошти з вашого банківського рахунку',
                en: 'On this page, you can withdraw funds from your bank account',
                ru: 'На этой странице вы можете снять средства с вашего банковского счёта',
                de: 'Auf dieser Seite kannst du Geld von deinem Bankkonto abheben',
                pl: 'Na tej stronie możesz wypłacić środki z swojego konta bankowego',
                zh: '在这个页面上，你可以从你的银行账户中提取资金',
            },
        },
        contributions: {
            name: {
                ua: 'Вклади',
                en: 'Contributions',
                ru: 'Вклады',
                de: 'Beiträge',
                pl: 'Wpłaty',
                zh: '交费',
            },
            desc: {
                ua: 'Кожен день ви будете отримувати на банківський баланс 1% від балансу депозиту',
                en: 'Every day, you will receive 1% of your deposit balance to your bank account',
                ru: 'Каждый день вы будете получать на банковский баланс 1% от баланса депозита',
                de: 'Jeden Tag erhältst du 1% deines Einzahlungsbetrags auf dein Bankkonto',
                pl: 'Codziennie otrzymasz 1% salda depozytu na swoje konto bankowe',
                zh: '每天，您将收到存款余额的1%到您的银行账户',
            },
        },
        transfers: {
            name: {
                ua: 'Перекази',
                en: 'Transfers',
                ru: 'Переводы',
                de: 'Überweisungen',
                pl: 'Przelewy',
                zh: '中转',
            },
            desc: {
                ua: 'На цій сторінці ви можете переказувати гроші на рахунки інших гравців',
                en: "On this page, you can transfer money to other players' accounts",
                ru: 'На этой странице вы можете переводить деньги на счета других игроков',
                de: 'Auf dieser Seite kannst du Geld auf die Konten anderer Spieler überweisen',
                pl: 'Na tej stronie możesz przekazywać pieniądze na konta innych graczy',
                zh: '在这个页面，你可以把钱转到其他玩家的账户',
            },
        },
        tax: {
            name: {
                ru: 'Комунальные платежи',
                ua: 'Комунальні платежі',
                en: 'Utility Bills',
                de: 'Nebenkosten',
                pl: 'Opłaty komunalne',
                zh: '水电费',
            },
            desc: {
                ru: 'На этой странице у вас есть отдельный баланс, который используется для оплаты домов, на деле всё просто, вы пополняете баланс а при квартплате недвижимости деньги отсюда автоматически снимаются',
                ua: 'На цій сторінці у вас є окремий баланс, який використовується для оплати будинків, насправді все просто, ви поповнюєте баланс а при квартплаті нерухомості гроші звідси автоматично знімаються',
                en: "This page provides you with a separate balance used for house payments. In reality, it's simple: you top up the balance, and the money is automatically deducted from it when paying property bills.",
                de: 'Auf dieser Seite hast du ein separates Guthaben, das für Hauszahlungen verwendet wird. Im Grunde ist es einfach: Du füllst das Guthaben auf, und das Geld wird automatisch abgebucht, wenn du die Immobilienrechnungen bezahlst.',
                pl: 'Na tej stronie masz osobiste saldo, które jest używane do płatności za dom. W rzeczywistości jest to proste: doładowujesz saldo, a pieniądze są automatycznie pobierane z niego przy płaceniu rachunków za nieruchomość.',
                zh: '这里用于存入房屋税款',
            },
        },
        commercial: {
            name: {
                ru: 'Коммерческий счёт',
                ua: 'Комерційний рахунок',
                en: 'Commercial Account',
                de: 'Geschäftskonto',
                pl: 'Konto handlowe',
                zh: '厂商价款',
            },
            desc: {
                ru: 'Этот счёт нужен для оплаты бизнесов любого вида как например банкомат или же 24/7. Отсюда каждый день автоматичкески будут отниматся деньги за ваши бизнесы',
                ua: 'Цей рахунок потрібен для оплати бізнесів будь-якого виду, наприклад банкомат або ж 24/7. Звідси щодня автоматично будуть зніматися гроші за ваші бізнеси',
                en: 'This account is needed for paying businesses of any kind such as ATMs or 24/7 stores. Money for your businesses will be automatically deducted from here every day',
                de: 'Dieses Konto wird für die Bezahlung von Geschäften jeglicher Art benötigt, z. B. Geldautomaten oder 24/7-Geschäfte. Geld für deine Geschäfte wird hier jeden Tag automatisch abgezogen',
                pl: 'To konto jest potrzebne do opłacania firm wszelkiego rodzaju, takich jak bankomaty czy sklepy 24/7. Pieniądze za twoje firmy będą automatycznie pobierane stąd każdego dnia',
                zh: '这个账户用于支付各种类型的企业，比如ATM机或24小时营业的商店。您的企业所需资金将每天自动从该账户中扣除。',
            },
        },
        phone: {
            name: {
                ru: 'Мобильный телефон',
                ua: 'Мобільний телефон',
                en: 'Mobile phone',
                de: 'Mobiltelefon',
                pl: 'Telefon komórkowy',
            },
            desc: {
                ru: 'На этой странице вы можете пополнить мобильный телефон',
                ua: 'На цій сторінці ви можете поповнити мобільний телефон',
                en: 'On this page, you can top up your mobile phone',
                de: 'Auf dieser Seite kannst du dein Mobiltelefon aufladen',
                pl: 'Na tej stronie możesz doładować swój telefon komórkowy',
            },
        },
        exit: {
            name: {
                ua: 'Вийти',
                en: 'Exit',
                ru: 'Уйти',
                de: 'Beenden',
                pl: 'Wyjście',
                zh: '退出',
            },
        },
    },
    card: {
        owner: {
            ua: 'Власник карти',
            en: 'Card Owner',
            ru: 'Владелец карты',
            de: 'Karteninhaber',
            pl: 'Właściciel karty',
            zh: '持卡人',
        },
        balance: {
            ua: 'Баланс',
            en: 'Balance',
            ru: 'Баланс',
            de: 'Kontostand',
            pl: 'Saldo',
            zh: '余款',
        },
    },
    control: {
        availableActions: {
            ua: 'Доступні дії',
            en: 'Available Actions',
            ru: 'Доступные действия',
            de: 'Verfügbare Aktionen',
            pl: 'Dostępne akcje',
            zh: '可用操作',
        },
        history: {
            ua: 'Історія транзакцій',
            en: 'Transaction History',
            ru: 'История транзакций',
            de: 'Transaktionsverlauf',
            pl: 'Historia transakcji',
            zh: '交易历史',
        },
    },
    historyDetail: {
        titleIncome: {
            ru: 'Поступление',
            ua: 'Надходження',
            en: 'Incoming',
            de: 'Eingang',
            pl: 'Wpływ',
            zh: '收入',
        },
        titleExpense: {
            ru: 'Списание',
            ua: 'Списання',
            en: 'Outgoing',
            de: 'Abbuchung',
            pl: 'Wypływ',
            zh: '支出',
        },
        dateTime: {
            ru: 'Дата и время',
            ua: 'Дата та час',
            en: 'Date and time',
            de: 'Datum und Uhrzeit',
            pl: 'Data i godzina',
            zh: '日期和时间',
        },
        counterparty: {
            ru: 'Контрагент',
            ua: 'Контрагент',
            en: 'Counterparty',
            de: 'Gegenpartei',
            pl: 'Kontrahent',
            zh: '对方',
        },
        accountNumber: {
            ru: 'Номер счёта',
            ua: 'Номер рахунку',
            en: 'Account number',
            de: 'Kontonummer',
            pl: 'Numer konta',
            zh: '账号',
        },
        source: {
            ru: 'Источник',
            ua: 'Джерело',
            en: 'Source',
            de: 'Quelle',
            pl: 'Źródło',
            zh: '来源',
        },
        sourceLabel: {
            atm: {
                ru: 'Банкомат',
                ua: 'Банкомат',
                en: 'ATM',
                de: 'Geldautomat',
                pl: 'Bankomat',
                zh: 'ATM',
            },
            bank: {
                ru: 'Банк',
                ua: 'Банк',
                en: 'Bank',
                de: 'Bank',
                pl: 'Bank',
                zh: '银行',
            },
            salary: {
                ru: 'Зарплата',
                ua: 'Зарплата',
                en: 'Salary',
                de: 'Gehalt',
                pl: 'Wypłata',
                zh: '工资',
            },
            fine: {
                ru: 'Штраф',
                ua: 'Штраф',
                en: 'Fine',
                de: 'Bußgeld',
                pl: 'Mandat',
                zh: '罚款',
            },
            shop: {
                ru: 'Магазин',
                ua: 'Магазин',
                en: 'Shop',
                de: 'Geschäft',
                pl: 'Sklep',
                zh: '商店',
            },
            job: {
                ru: 'Работа',
                ua: 'Робота',
                en: 'Job',
                de: 'Arbeit',
                pl: 'Praca',
                zh: '工作',
            },
            system: {
                ru: 'Система',
                ua: 'Система',
                en: 'System',
                de: 'System',
                pl: 'System',
                zh: '系统',
            },
            tax: {
                ru: 'Налоги',
                ua: 'Податки',
                en: 'Taxes',
                de: 'Steuern',
                pl: 'Podatki',
                zh: '税务',
            },
            deposit: {
                ru: 'Вклад',
                ua: 'Вклад',
                en: 'Deposit',
                de: 'Einlage',
                pl: 'Depozyt',
                zh: '存款',
            },
            commercial: {
                ru: 'Коммерческий счёт',
                ua: 'Комерційний рахунок',
                en: 'Commercial account',
                de: 'Geschäftskonto',
                pl: 'Konto firmowe',
                zh: '商业账户',
            },
            phone: {
                ru: 'Мобильный',
                ua: 'Мобільний',
                en: 'Mobile',
                de: 'Mobiltelefon',
                pl: 'Telefon',
                zh: '手机',
            },
            rent: {
                ru: 'Аренда',
                ua: 'Оренда',
                en: 'Rent',
                de: 'Miete',
                pl: 'Wynajem',
                zh: '租金',
            },
            business: {
                ru: 'Бизнес',
                ua: 'Бізнес',
                en: 'Business',
                de: 'Geschäft',
                pl: 'Biznes',
                zh: '生意',
            },
            house: {
                ru: 'Дом',
                ua: 'Будинок',
                en: 'House',
                de: 'Haus',
                pl: 'Dom',
                zh: '房子',
            },
        },
        atmId: {
            ru: 'Банкомат #{{id}}',
            ua: 'Банкомат #{{id}}',
            en: 'ATM #{{id}}',
            de: 'Geldautomat #{{id}}',
            pl: 'Bankomat #{{id}}',
            zh: 'ATM #{{id}}',
        },
        comment: {
            ru: 'Комментарий',
            ua: 'Коментар',
            en: 'Comment',
            de: 'Kommentar',
            pl: 'Komentarz',
            zh: '备注',
        },
        repeat: {
            ru: 'Повторить',
            ua: 'Повторити',
            en: 'Repeat',
            de: 'Wiederholen',
            pl: 'Powtórz',
            zh: '重复',
        },
        filterAll: {
            ru: 'Все',
            ua: 'Усі',
            en: 'All',
            de: 'Alle',
            pl: 'Wszystkie',
            zh: '全部',
        },
        filterIncome: {
            ru: 'Поступления',
            ua: 'Надходження',
            en: 'Income',
            de: 'Eingang',
            pl: 'Wpływy',
            zh: '收入',
        },
        filterExpense: {
            ru: 'Списания',
            ua: 'Списання',
            en: 'Expense',
            de: 'Abbuchung',
            pl: 'Wypływy',
            zh: '支出',
        },
        empty: {
            ru: 'Операций пока нет',
            ua: 'Операцій поки немає',
            en: 'No transactions yet',
            de: 'Noch keine Transaktionen',
            pl: 'Brak transakcji',
            zh: '暂无交易',
        },
        loading: {
            ru: 'Загрузка...',
            ua: 'Завантаження...',
            en: 'Loading...',
            de: 'Lädt...',
            pl: 'Ładowanie...',
            zh: '加载中...',
        },
        endReached: {
            ru: 'Вся история загружена',
            ua: 'Усю історію завантажено',
            en: 'End of history',
            de: 'Verlauf vollständig geladen',
            pl: 'Koniec historii',
            zh: '已加载全部历史',
        },
    },
    withdraw: {
        input: {
            ua: 'Введіть суму до отримання',
            en: 'Enter amount to receive',
            ru: 'Введите сумму к получению',
            de: 'Betrag zur Auszahlung eingeben',
            pl: 'Wprowadź kwotę do otrzymania',
            zh: '输入到账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },

        button: {
            ua: 'Зняти',
            en: 'Withdraw',
            ru: 'Снять',
            de: 'Abheben',
            pl: 'Wypłać',
            zh: '撤销',
        },
    },
    deposit: {
        input: {
            ua: 'Введіть суму до зарахування',
            en: 'Enter amount to deposit',
            ru: 'Введите сумму к зачислению',
            de: 'Betrag zur Gutschrift eingeben',
            pl: 'Wprowadź kwotę do wpłaty',
            zh: '输入入账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },
        button: {
            ua: 'Внести',
            en: 'Deposit',
            ru: 'Внести',
            de: 'Einzahlen',
            pl: 'Wpłać',
            zh: '存款',
        },
    },
    contributions: {
        info: {
            first: {
                ru: 'Вы получите проценты если отыграете 15 минут за день',
                en: 'You will receive interest if you play for 15 minutes a day.',
                ua: 'Ви отримаєте відсотки, якщо відіграєте 15 хвилин на день',
                de: 'Du erhältst Zinsen, wenn du jeden Tag mindestens 15 Minuten spielst.',
                pl: 'Otrzymasz odsetki, jeśli będziesz grał przez 15 minut dziennie.',
                zh: '如果你每天玩15分钟，你就会获得利息。',
            },
            second: {
                ru: 'Как только вы отыграете 15 минут депозит автоматически начислится',
                en: 'Once you play for 15 minutes, the deposit will be automatically credited.',
                ua: 'Як тільки ви відіграєте 15 хвилин, депозит автоматично зарахується',
                de: 'Sobald du 15 Minuten gespielt hast, wird die Einzahlung automatisch gutgeschrieben.',
                pl: 'Jak tylko zagrasz przez 15 minut, wpłata zostanie automatycznie zaksięgowana.',
                zh: '游玩15分钟后，利息将自动存入账户',
            },
            third: {
                ru: 'Процент на снятие денег с баланса депозита 15%',
                en: 'The percentage for withdrawing money from the deposit balance is 15%',
                ua: 'Відсоток на зняття грошей з балансу депозиту 15%',
                de: 'Der Prozentsatz für die Auszahlung von Geldern vom Einzahlungsguthaben beträgt 15%',
                pl: 'Odsetek od wypłaty pieniędzy z salda depozytu wynosi 15%',
                zh: '从存款余额中支取的比例是15%',
            },
            four: {
                ru: 'Максимальный баланс вкладов ${{sum}}',
                ua: 'Максимальний баланс вкладів ${{sum}}',
                en: 'Maximum deposit balance ${{sum}}',
                de: 'Maximales Einlageguthaben ${{sum}}',
                pl: 'Maksymalne saldo depozytów ${{sum}}',
            },
        },
        yourBalance: {
            ru: 'Ваш баланс вкладов',
            en: 'Your deposit balance',
            ua: 'Ваш баланс вкладів',
            de: 'Dein Einzahlungsguthaben',
            pl: 'Twój bilans depozytowy',
            zh: '您的存款余额',
        },
        input: {
            ru: 'Введите сумму',
            en: 'Enter amount',
            ua: 'Введіть суму',
            de: 'Gib den Betrag ein',
            pl: 'Wprowadź kwotę',
            zh: '输入金额',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        withdraw: {
            ru: 'Забрать',
            en: 'Withdraw',
            ua: 'Забрати',
            de: 'Abheben',
            pl: 'Wypłata',
            zh: '取款',
        },
        deposit: {
            ru: 'Внести',
            en: 'Deposit',
            ua: 'Внести',
            de: 'Einzahlen',
            pl: 'Wpłata',
            zh: '存款',
        },
        timeLeft: {
            ru: 'Времени до получения процентов: {{timeLeftToDeposit}}',
            en: 'Time left until interest accrual: {{timeLeftToDeposit}}',
            ua: 'Часу до отримання відсотків: {{timeLeftToDeposit}}',
            de: 'Verbleibende Zeit bis zur Zinsgutschrift: {{timeLeftToDeposit}}',
            pl: 'Czas do naliczenia odsetek: {{timeLeftToDeposit}}',
            zh: '计息前的剩余时间:{{timeLeftToDeposit}}',
        },
        timeLeftLabel: {
            ru: 'Времени до получения процентов',
            en: 'Time left until interest accrual',
            ua: 'Часу до отримання відсотків',
            de: 'Verbleibende Zeit bis zur Zinsgutschrift',
            pl: 'Czas do naliczenia odsetek',
            zh: '计息前的剩余时间',
        },
        payoutReceivedToday: {
            ru: 'Выплата процентов за сегодня уже получена',
            en: 'Today interest payout has already been received',
            ua: 'Виплату відсотків за сьогодні вже отримано',
            de: 'Die Zinsgutschrift fur heute wurde bereits erhalten',
            pl: 'Dzisiejsza wyplata odsetek zostala juz odebrana',
            zh: '今天的利息发放已领取',
        },
    },
    transfers: {
        commentPlaceholder: {
            ru: 'Комментарий (необязательно)',
            ua: 'Коментар (необов’язково)',
            en: 'Comment (optional)',
            de: 'Kommentar (optional)',
            pl: 'Komentarz (opcjonalnie)',
            zh: '备注（可选）',
        },
        friendsHint: {
            ru: 'Начните вводить имя друга или номер счёта',
            ua: 'Почніть вводити імʼя друга або номер рахунку',
            en: 'Start typing a friend name or account number',
            de: 'Tippe einen Freundnamen oder Kontonummer ein',
            pl: 'Zacznij wpisywać imię znajomego lub numer konta',
            zh: '输入好友姓名或账号',
        },
        friendsEmpty: {
            ru: 'Совпадений не найдено',
            ua: 'Збігів не знайдено',
            en: 'No matches found',
            de: 'Keine Treffer',
            pl: 'Brak wyników',
            zh: '没有匹配项',
        },
        lookupSearching: {
            ru: 'Поиск счёта...',
            ua: 'Пошук рахунку...',
            en: 'Searching account...',
            de: 'Konto wird gesucht...',
            pl: 'Wyszukiwanie konta...',
            zh: '查找账户...',
        },
        friendOnline: {
            ru: 'онлайн',
            ua: 'онлайн',
            en: 'online',
            de: 'online',
            pl: 'online',
            zh: '在线',
        },
        friendOffline: {
            ru: 'офлайн',
            ua: 'офлайн',
            en: 'offline',
            de: 'offline',
            pl: 'offline',
            zh: '离线',
        },
        commentInvalidChars: {
            ru: 'Недопустимые символы в комментарии',
            ua: 'Недопустимі символи в коментарі',
            en: 'Invalid characters in comment',
            de: 'Ungültige Zeichen im Kommentar',
            pl: 'Niedozwolone znaki w komentarzu',
            zh: '备注含非法字符',
        },
        commentTooLong: {
            ru: 'Комментарий слишком длинный',
            ua: 'Коментар занадто довгий',
            en: 'Comment is too long',
            de: 'Kommentar ist zu lang',
            pl: 'Komentarz jest za długi',
            zh: '备注过长',
        },
        input: {
            ua: 'Введіть суму переказу',
            en: 'Enter transfer amount',
            ru: 'Введите сумму перевода',
            de: 'Überweisungsbetrag eingeben',
            pl: 'Wprowadź kwotę przelewu',
            zh: '输入转账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        inputAccount: {
            ru: 'Введите номер счёта',
            en: 'Enter account number',
            ua: 'Введіть номер рахунку',
            de: 'Gib die Kontonummer ein',
            pl: 'Wprowadź numer konta',
            zh: '重复帐户号码',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },
        button: {
            ru: 'Перевести',
            en: 'Transfer',
            ua: 'Переказати',
            de: 'Uberweisen',
            pl: 'Przelej',
            zh: '转账',
        },
    },
    info: {
        bankomat: {
            ru: 'Банкомат',
            en: 'ATM',
            ua: 'Банкомат',
            de: 'Geldautomat',
            pl: 'Bankomat',
            zh: 'ATM',
        },
        owner: {
            ru: 'Владелец банкомата',
            en: 'ATM Owner',
            ua: 'Власник банкомата',
            de: 'Geldautomat Besitzer',
            pl: 'Właściciel bankomatu',
            zh: 'ATM所有者',
        },
        price: {
            ru: 'Гос. цена',
            en: 'Government Price',
            ua: 'Державна ціна',
            de: 'Staatspreis',
            pl: 'Cena rządowa',
            zh: '计划价格',
        },
        service: {
            ru: 'Обслуживание',
            ua: 'Обслуговування',
            en: 'Service',
            de: 'Wartung',
            pl: 'Usługa',
            zh: '服务',
        },
        perDay: {
            ru: 'в день',
            ua: 'в день',
            en: 'per day',
            de: 'pro Tag',
            pl: 'na dzień',
            zh: '每日',
        },
        comission: {
            ru: 'Текущая комиссия',
            en: 'Current Commission',
            ua: 'Поточна комісія',
            de: 'Aktuelle Provision',
            pl: 'Aktualna prowizja',
            zh: '代理商',
        },
        revenue: {
            ru: 'Выручка',
            en: 'Revenue',
            ua: 'Виручка',
            de: 'Einnahmen',
            pl: 'Dochód',
            zh: '收入',
        },
        operationsPerDay: {
            ru: 'Операций за день',
            en: 'Operations per Day',
            ua: 'Операцій за день',
            de: 'Operationen pro Tag',
            pl: 'Operacje dziennie',
            zh: '每日操作次数',
        },
        buy: {
            ru: 'Купить',
            en: 'Buy',
            ua: 'Купити',
            de: 'Kaufen',
            pl: 'Kupić',
            zh: '买',
        },
        sell: {
            ru: 'Продать',
            en: 'Sell',
            ua: 'Продати',
            de: 'Verkaufen',
            pl: 'Sprzedać',
            zh: '卖',
        },
        withdrawMoney: {
            ru: 'Снять деньги',
            en: 'Withdraw Money',
            ua: 'Зняти гроші',
            de: 'Geld abheben',
            pl: 'Wypłacić pieniądze',
            zh: '取款',
        },
        desc: {
            ru: `Приобретая банкомат, вы открываете себе уникальную возможность получать прибыль, которая будет поступать на ваш счет с каждой операцией,
                        проводимой в этом банкомате. Это представляет собой весьма выгодное вложение, поскольку каждая транзакция становится источником дохода.
                        Представьте себе это как надежный источник пассивного дохода, который будет приносить вам дивиденды со временем.`,
            en: `By purchasing an ATM, you open yourself a unique opportunity to receive profit that will be credited to your account with each operation
                        carried out in this ATM. This is a very profitable investment, as each transaction becomes a source of income.
                        Imagine this as a reliable source of passive income that will bring you dividends over time.`,
            ua: `Придбавши банкомат, ти відкриваєш собі унікальну можливість отримувати прибуток, який буде надходити на твій рахунок з кожною операцією,
                        проведеною у цьому банкоматі. Це дуже вигідне вкладення, оскільки кожна транзакція стає джерелом доходу.
                        Уяви собі це як надійний джерело пасивного доходу, яке буде приносити тобі дивіденди з часом.`,
            de: `Durch den Kauf eines Geldautomaten eröffnest du dir eine einzigartige Möglichkeit, Gewinn zu erzielen, der mit jeder durchgeführten Transaktion auf dein Konto gutgeschrieben wird.
                        Dies ist eine sehr rentable Investition, da jede Transaktion zu einer Einkommensquelle wird.
                        Stelle dir dies als zuverlässige Quelle passiven Einkommens vor, die dir im Laufe der Zeit Dividenden bringt.`,
            pl: `Kupując bankomat, otwierasz sobie unikalną możliwość uzyskiwania zysku, który będzie wpływał na twoje konto z każdą przeprowadzoną operacją
                        w tym bankomacie. Jest to bardzo opłacalna inwestycja, ponieważ każda transakcja staje się źródłem dochodu.
                        Wyobraź sobie to jako niezawodne źródło pasywnego dochodu, które przyniesie ci dywidendy w ciągu czasu.`,
            zh: '通过购买一台ATM机，您为自己开辟了一条独特的获取利润的途径，每执行一次ATM机上的交易，您的账户就会增加一笔收入。这是一项非常有利可图的投资，因为每次交易都是收入的来源。',
        },
        goverment: {
            ru: 'Государство',
            en: 'Government',
            ua: 'Держава',
            de: 'Regierung',
            pl: 'Rząd',
            zh: '政府',
        },
        modal: {
            title: {
                ru: 'Продажа банкомата',
                en: 'ATM Sale',
                ua: 'Продаж банкомату',
                de: 'Geldautomatenverkauf',
                pl: 'Sprzedaż bankomatu',
                zh: 'ATM销售',
            },
            message: {
                ru: 'Вы действительно хотите продать банкомат за ${{price}} ?',
                en: 'Do you really want to sell the ATM for ${{price}}?',
                ua: 'Ви дійсно бажаєте продати банкомат за ${{price}} ?',
                de: 'Möchtest du den Geldautomaten wirklich für ${{price}} verkaufen?',
                pl: 'Czy naprawdę chcesz sprzedać bankomat za ${{price}}?',
                zh: '"你真的想以{{price}}的价格卖掉这台ATM吗？"',
            },
            confirmButton: {
                ru: 'Продать',
                en: 'Sell',
                ua: 'Продати',
                de: 'Verkaufen',
                pl: 'Sprzedaj',
                zh: '卖',
            },
            cancelButton: {
                ru: 'Отмена',
                en: 'Cancel',
                ua: 'Відміна',
                de: 'Abbrechen',
                pl: 'Anuluj',
                zh: '取消',
            },
        },
    },
    tax: {
        balance: {
            ru: 'Баланс',
            en: 'Balance',
            ua: 'Баланс',
            de: 'Guthaben',
            pl: 'Saldo',
            zh: '平衡',
        },
        input: {
            ru: 'Введите сумму к зачислению',
            en: 'Enter amount to deposit',
            ua: 'Введіть суму до зарахування',
            de: 'Betrag zur Gutschrift eingeben',
            pl: 'Wprowadź kwotę do wpłaty',
            zh: '输入入账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },
        button: {
            ru: 'Пополнить',
            en: 'Deposit',
            ua: 'Поповнити',
            de: 'Einzahlen',
            pl: 'Wpłać',
            zh: '存款',
        },
    },
    commercial: {
        balance: {
            ru: 'Баланс',
            en: 'Balance',
            ua: 'Баланс',
            de: 'Guthaben',
            pl: 'Saldo',
            zh: '平衡',
        },
        input: {
            ru: 'Введите сумму к зачислению',
            en: 'Enter amount to deposit',
            ua: 'Введіть суму до зарахування',
            de: 'Betrag zur Gutschrift eingeben',
            pl: 'Wprowadź kwotę do wpłaty',
            zh: '输入入账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },
        button: {
            ru: 'Пополнить',
            en: 'Deposit',
            ua: 'Поповнити',
            de: 'Einzahlen',
            pl: 'Wpłać',
            zh: '存款',
        },
    },
    phone: {
        balance: {
            ru: 'Баланс',
            en: 'Balance',
            ua: 'Баланс',
            de: 'Guthaben',
            pl: 'Saldo',
            zh: '平衡',
        },
        input: {
            ru: 'Введите сумму к зачислению',
            en: 'Enter amount to deposit',
            ua: 'Введіть суму до зарахування',
            de: 'Betrag zur Gutschrift eingeben',
            pl: 'Wprowadź kwotę do wpłaty',
            zh: '输入入账金额',
        },
        max: {
            ua: 'MAX',
            en: 'MAX',
            ru: 'MAX',
            de: 'MAX',
            pl: 'MAX',
            zh: '最大',
        },
        commission: {
            ua: 'Комісія',
            en: 'Commission',
            ru: 'Комиссия',
            de: 'Kommission',
            pl: 'Prowizja',
            zh: '支付手续费',
        },
        commissionAmount: {
            ua: 'Комісія, $',
            en: 'Commission, $',
            ru: 'Комиссия, $',
            de: 'Gebühr, $',
            pl: 'Prowizja, $',
            zh: '手续费, $',
        },
        totalDebit: {
            ua: 'До списання',
            en: 'Total debit',
            ru: 'К списанию',
            de: 'Gesamtbelastung',
            pl: 'Do pobrania',
            zh: '总扣款',
        },
        total: {
            ua: 'Загальна сума',
            en: 'Total Amount',
            ru: 'Итоговая сумма',
            de: 'Gesamtbetrag',
            pl: 'Całkowita kwota',
            zh: '总金额',
        },
        validation: {
            invalidAmount: {
                ua: 'Введіть коректну суму',
                en: 'Enter a valid amount',
                ru: 'Введите корректную сумму',
                de: 'Gib einen korrekten Betrag ein',
                pl: 'Wprowadź poprawną kwotę',
                zh: '请输入正确金额',
            },
            notEnoughWithCommission: {
                ua: 'Недостатньо коштів з урахуванням комісії',
                en: 'Insufficient funds including commission',
                ru: 'Недостаточно средств с учетом комиссии',
                de: 'Nicht genug Guthaben inklusive Gebühr',
                pl: 'Niewystarczające środki z prowizją',
                zh: '余额不足（含手续费）',
            },
            targetLimitExceeded: {
                ua: 'Перевищено ліміт цільового балансу',
                en: 'Target balance limit exceeded',
                ru: 'Превышен лимит целевого баланса',
                de: 'Ziellimit überschritten',
                pl: 'Przekroczono limit salda docelowego',
                zh: '超出目标余额上限',
            },
        },
        button: {
            ru: 'Пополнить',
            en: 'Deposit',
            ua: 'Поповнити',
            de: 'Einzahlen',
            pl: 'Wpłać',
            zh: '存款',
        },
        noSimCard: {
            ru: '🍃 У вас нет сим-карты',
            ua: '🍃 У вас немає SIM-картки',
            en: "🍃 You don't have a SIM card",
            de: '🍃 Du hast keine SIM-Karte',
            pl: '🍃 Nie masz karty SIM',
        },
    },
};
