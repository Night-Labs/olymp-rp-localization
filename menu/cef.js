import { settings } from './settings';
import { inventory } from './inventory';
import { macros } from './macros';

export const cef = {
    items: {
        main: {
            ua: 'Головна',
            en: 'Main',
            ru: 'Главная',
            de: 'Startseite',
            pl: 'Główna',
            zh: '主要的',
        },
        stats: {
            ua: 'Статистика',
            en: 'Stats',
            ru: 'Статистика',
            de: 'Statistiken',
            pl: 'Statystyki',
            zh: '统计',
        },
        inventory: {
            ua: 'Інвентар',
            en: 'Inventory',
            ru: 'Инвентарь',
            de: 'Inventar',
            pl: 'Inwentarz',
            zh: '库存',
        },
        battlepass: {
            ru: 'Баттл пасс',
            ua: 'Бойовий пропуск',
            en: 'Battle Pass',
            de: 'Battle Pass',
            pl: 'Karnet bojowy',
            zh: '战斗通行证',
        },
        donate: {
            ua: 'Магазин',
            en: 'Shop',
            ru: 'Магазин',
            de: 'Shop',
            pl: 'Sklep',
            zh: '商店',
        },
        settings: {
            ua: 'Налаштування',
            en: 'Settings',
            ru: 'Настройки',
            de: 'Einstellungen',
            pl: 'Ustawienia',
            zh: '设置',
        },
        quests: {
            ua: 'Лінія квестів',
            en: 'Quests',
            ru: 'Квестовая линия',
            de: 'Quests',
            pl: 'Zadania',
            zh: '任务',
        },
        achievements: {
            ua: 'Досягнення',
            en: 'Achievements',
            ru: 'Достижения',
            de: 'Erfolge',
            pl: 'Osiągnięcia',
            zh: '成就',
        },
        faction: {
            ru: 'Фракция',
            ua: 'Фракція',
            en: 'Faction',
            de: 'Fraktion',
            pl: 'Frakcja',
            zh: '帮派',
        },
        crash: {
            ru: 'Краш режим',
            en: 'Crash mode',
            ua: 'Режим крашу',
            de: 'Absturzmodus',
            pl: 'Tryb awarii',
            zh: '碰撞模式',
        },
        referral: {
            ru: 'Рефералы',
            ua: 'Реферали',
            en: 'Referrals',
            de: 'Empfehlungen',
            pl: 'Polecenia',
            zh: '引荐',
        },
    },
    main: {
        title: {
            ua: `Ласкаво просимо на`,
            en: `Welcome to `,
            ru: `Добро пожаловать на `,
            de: `Willkommen bei `,
            pl: `Witaj w `,
            zh: `欢迎到来 `,
        },
        subtitle: {
            ua: 'Наш сервер розробляється повністю з нуля. Він знаходиться у активній розробці та постійно розвиватиметься. Ми сподіваємося на вашу підтримку.',
            en: 'Our server is being developed from scratch. It is in active development and will continue to evolve. We rely on your support.',
            ru: 'Наш сервер разрабатывается полностью с нуля. Он находится в активной разработке и будет постоянно развиваться. Мы надеемся на вашу поддержку.',
            de: 'Unser Server wird von Grund auf neu entwickelt. Er befindet sich in aktiver Entwicklung und wird kontinuierlich weiter ausgebaut. Wir zählen auf deine Unterstützung.',
            pl: 'Nasz serwer jest rozwijany od podstaw. Jest w trakcie aktywnego rozwoju i będzie się stale rozwijał. Liczymy na wsparcie.',
            zh: '我们的服务器是从头开始开发的。它正在积极发展，并将继续发展。我们依靠你的支持',
        },
        changes: {
            ru: 'Последние новости:',
            ua: 'Останні новини:',
            en: 'Latest news:',
            de: 'Neueste Nachrichten:',
            pl: 'Najnowsze wiadomości:',
        },
        discord: {
            ua: 'Наш Discord',
            en: 'Our Discord',
            ru: 'Наш дискорд',
            de: 'Unser Discord',
            pl: 'Nasz Discord',
            zh: '我们不和谐',
        },
        copied: {
            ua: 'Скопійовано',
            en: 'Copied',
            ru: 'Скопировано',
            de: 'Kopiert',
            pl: 'Skopiowane',
            zh: '复制',
        },
        site: {
            ua: 'Наш сайт',
            en: 'Our website',
            ru: 'Наш сайт',
            de: 'Unsere Website',
            pl: 'Nasza strona internetowa',
            zh: '我们网站',
        },

        notifyHeader: {
            ru: 'Главная',
            ua: 'Головна',
            en: 'Home',
            de: 'Startseite',
            pl: 'Główna',
            zh: '主页',
        },
        notifyText: {
            ru: 'Ссылка скопирована, пожалуйста, вставь её в браузер',
            ua: 'Посилання скопійовано, будь ласка, встав його в браузер',
            en: 'Link copied, please paste it into your browser',
            de: 'Link kopiert. Bitte füge ihn in deinen Browser ein.',
            pl: 'Link skopiowany, proszę wklej go do przeglądarki',
        },
    },
    stats: {
        title: {
            ua: 'Статистика',
            en: 'Statistics',
            ru: 'Статистика',
            de: 'Statistiken',
            pl: 'Statystyki',
            zh: '统计',
        },
        subtitle: {
            ua: 'Тут описана ваша статистика',
            en: 'Here is your statistics',
            ru: 'Здесь описана ваша статистика',
            de: 'Hier sind deine Statistiken',
            pl: 'Oto twoje statystyki',
            zh: '这是你的统计数据',
        },
        notifyHeader: {
            ru: 'GPS',
            ua: 'GPS',
            en: 'GPS',
            de: 'GPS',
            pl: 'GPS',
            zh: 'GPS',
        },
        routeSetNotify: {
            ru: 'Маршрут проложен',
            ua: 'Маршрут прокладено',
            en: 'Route has been set',
            de: 'Route wurde festgelegt',
            pl: 'Trasa została wyznaczona',
            zh: '路线已设置',
        },

        mainStats: {
            ua: 'Загальна статистика',
            en: 'Overall Statistics',
            ru: 'Общая статистика',
            de: 'Gesamtstatistik',
            pl: 'Statystyki ogólne',
            zh: '全面统计',
        },
        jobs: {
            ua: 'Рівні робіт',
            en: 'Job Levels',
            ru: 'Уровни работ',
            de: 'Job-Levels',
            pl: 'Poziomy pracy',
            zh: '工作等级',
        },
        property: {
            ua: 'Майно',
            en: 'Property',
            ru: 'Имущество',
            de: 'Eigentum',
            pl: 'Nieruchomość',
            zh: '属性',
        },

        propertyList: {
            house: {
                ru: 'Дом',
                ua: 'Будинок',
                en: 'House',
                de: 'Haus',
                pl: 'Dom',
            },
        },

        mainStatsList: {
            gender: {
                ua: 'Стать',
                en: 'Gender',
                ru: 'Пол',
                de: 'Geschlecht',
                pl: 'Płeć',
                zh: '性别',
            },
            age: {
                ua: 'Вік',
                en: 'Age',
                ru: 'Возраст',
                de: 'Alter',
                pl: 'Wiek',
                zh: '年龄',
            },
            id: {
                ua: 'ID',
                en: 'ID',
                ru: 'ID',
                de: 'ID',
                pl: 'ID',
                zh: 'ID',
            },
            cash: {
                ua: 'Готівка',
                en: 'Cash',
                ru: 'Наличные',
                de: 'Bargeld',
                pl: 'Gotówka',
                zh: '现金',
            },
            lvl: {
                ua: 'Рівень',
                en: 'Level',
                ru: 'Уровень',
                de: 'Level',
                pl: 'Poziom',
                zh: '等级',
            },
            exp: {
                ua: 'EXP',
                en: 'EXP',
                ru: 'EXP',
                de: 'EXP',
                pl: 'EXP',
                zh: '经验',
            },
            faction: {
                ua: 'Організація',
                en: 'Faction',
                ru: 'Организация',
                de: 'Fraktion',
                pl: 'Frakcja',
                zh: '帮派',
            },
            post: {
                ua: 'Посада',
                en: 'Position',
                ru: 'Должность',
                de: 'Position',
                pl: 'Stanowisko',
                zh: '位置',
            },
            additionalAge: {
                ua: 'років',
                en: 'years',
                ru: 'лет',
                de: 'Jahre',
                pl: 'lat',
                zh: '年后',
            },
            additionalPost: {
                ua: 'Не має',
                en: 'N/A',
                ru: 'Не состоит',
                de: 'Nicht zutreffend',
                pl: 'Brak',
                zh: 'N/A',
            },
            no: {
                ua: 'Немає',
                en: 'No',
                ru: 'Нет',
                de: 'Nein',
                pl: 'Nie',
                zh: '不',
            },
            phoneNumber: {
                ru: 'Номер телефона',
                ua: 'Номер телефону',
                en: 'Phone number',
                de: 'Telefonnummer',
                pl: 'Numer telefonu',
            },
            bank: {
                ru: 'Номер банковского счета',
                ua: 'Номер банківського рахунку',
                en: 'Bank account number',
                de: 'Bankkontonummer',
                pl: 'Numer rachunku bankowego',
            },
            acquaintances: {
                ru: 'Знакомые',
                ua: 'Знайомі',
                en: 'Acquaintances',
                de: 'Bekannte',
                pl: 'Znajomi',
            },
            registerDate: {
                ru: 'Дата регистрации аккаунта',
                ua: 'Дата реєстрації акаунта',
                en: 'Account registration date',
                de: 'Registrierungsdatum des Kontos',
                pl: 'Data rejestracji konta',
            },
            warn: {
                ru: 'Варны',
                ua: 'Попередження',
                en: 'Warnings',
                de: 'Verwarnungen',
                pl: 'Ostrzeżenia',
            },
            marriage: {
                ru: 'Семейное положение',
                ua: 'Сімейний стан',
                en: 'Marital status',
                de: 'Familienstand',
                pl: 'Stan cywilny',
            },
            notMarried: {
                ru: 'Не в браке',
                ua: 'Не в шлюбі',
                en: 'Not married',
                de: 'Nicht verheiratet',
                pl: 'Nie w związku małżeńskim',
            },
            marriedFemale: {
                ru: 'Замужем за',
                ua: 'Заміжня за',
                en: 'Married to',
                de: 'Verheiratet mit',
                pl: 'Zamężna za',
            },
            marriedMale: {
                ru: 'Женат на',
                ua: 'Одружений з',
                en: 'Married to',
                de: 'Verheiratet mit',
                pl: 'Żonaty z',
            },
        },
        categories: {
            all: {
                ua: 'Усе',
                en: 'All',
                ru: 'Всё',
                de: 'Alle',
                pl: 'Wszystko',
                zh: '全部',
            },
            vehicle: {
                ua: 'Транспорт',
                en: 'Vehicles',
                ru: 'Транспорт',
                de: 'Fahrzeuge',
                pl: 'Pojazdy',
                zh: '车辆',
            },
            house: {
                ua: 'Будинки',
                en: 'Houses',
                ru: 'Дома',
                de: 'Häuser',
                pl: 'Domki',
                zh: '房屋',
            },
        },
        wanted: {
            ru: 'Розыск',
            ua: 'Розшук',
            en: 'Wanted',
            de: 'Fahndung',
            pl: 'Poszukiwany',
        },

        online: {
            ru: 'Ваш онлайн',
            ua: 'Ваш онлайн',
            en: 'Your online',
            de: 'Ihr Online-Status',
            pl: 'Twój status online',
        },

        jobLevels: {
            ru: 'Уровни работ',
            ua: 'Рівні робіт',
            en: 'Job levels',
            de: 'Arbeitslevel',
            pl: 'Poziomy prac',
        },
        skills: {
            ru: 'Навыки',
            ua: 'Навички',
            en: 'Skills',
            de: 'Fähigkeiten',
            pl: 'Umiejętności',
        },
        condition: {
            ru: 'Состояние',
            ua: 'Стан',
            en: 'Condition',
            de: 'Zustand',
            pl: 'Stan',
        },
        effects: {
            ru: 'Эффекты',
            ua: 'Ефекти',
            en: 'Effects',
            de: 'Effekte',
            pl: 'Efekty',
        },
        vitals: {
            health: {
                ru: 'Здоровье',
                ua: "Здоров'я",
                en: 'Health',
                de: 'Gesundheit',
                pl: 'Zdrowie',
            },
            armour: {
                ru: 'Броня',
                ua: 'Броня',
                en: 'Armour',
                de: 'Rüstung',
                pl: 'Pancerz',
            },
            hunger: {
                ru: 'Сытость',
                ua: 'Ситість',
                en: 'Satiety',
                de: 'Sättigung',
                pl: 'Sytość',
            },
            thirst: {
                ru: 'Вода',
                ua: 'Вода',
                en: 'Hydration',
                de: 'Wasser',
                pl: 'Woda',
            },
            energy: {
                ru: 'Выносливость',
                ua: 'Витривалість',
                en: 'Stamina',
                de: 'Ausdauer',
                pl: 'Wytrzymałość',
            },
        },
        noModifiers: {
            ru: 'Нет активных эффектов',
            ua: 'Немає активних ефектів',
            en: 'No active effects',
            de: 'Keine aktiven Effekte',
            pl: 'Brak aktywnych efektów',
        },
        permanentEffect: {
            ru: 'Постоянно',
            ua: 'Постійно',
            en: 'Permanent',
            de: 'Dauerhaft',
            pl: 'Na stałe',
        },
        diseasesTitle: {
            ru: 'Заболевания',
            ua: 'Захворювання',
            en: 'Illnesses',
            de: 'Krankheiten',
            pl: 'Choroby',
        },
        noDiseases: {
            ru: 'Вы полностью здоровы',
            ua: 'Ви цілком здорові',
            en: 'You are perfectly healthy',
            de: 'Du bist vollkommen gesund',
            pl: 'Jesteś całkowicie zdrowy',
        },
        severity: {
            mild: {
                ru: 'Лёгкая стадия',
                ua: 'Легка стадія',
                en: 'Mild stage',
                de: 'Leichtes Stadium',
                pl: 'Łagodne stadium',
            },
            moderate: {
                ru: 'Умеренная стадия',
                ua: 'Помірна стадія',
                en: 'Moderate stage',
                de: 'Mittleres Stadium',
                pl: 'Umiarkowane stadium',
            },
            severe: {
                ru: 'Тяжёлая стадия',
                ua: 'Важка стадія',
                en: 'Severe stage',
                de: 'Schweres Stadium',
                pl: 'Ciężkie stadium',
            },
        },

        periodButtons: {
            daily: {
                ru: 'День',
                ua: 'День',
                en: 'Day',
                de: 'Tag',
                pl: 'Dzień',
            },
            weekly: {
                ru: 'Неделя',
                ua: 'Тиждень',
                en: 'Week',
                de: 'Woche',
                pl: 'Tydzień',
            },
            monthly: {
                ru: 'Месяц',
                ua: 'Місяць',
                en: 'Month',
                de: 'Monat',
                pl: 'Miesiąc',
            },
            total: {
                ru: 'Всего',
                ua: 'Всього',
                en: 'Total',
                de: 'Gesamt',
                pl: 'Łącznie',
            },
        },
        bankDeposit: {
            ru: 'Депозит в банке',
            ua: 'Банківський депозит',
            en: 'Bank deposit',
            de: 'Bankeinlage',
            pl: 'Depozyt bankowy',
        },
        promocodeTitle: {
            ru: 'Промокод {{name}}',
            ua: 'Промокод {{name}}',
            en: 'Promocode {{name}}',
            de: 'Promo-Code {{name}}',
            pl: 'Kod promocyjny {{name}}',
            zh: '促销代码 {{name}}',
        },
        promocodeTimeLeft: {
            ru: 'Осталось отыграть',
            ua: 'Залишилося відіграти',
            en: 'Time left to play',
            de: 'Noch zu spielen',
            pl: 'Pozostało do rozegrania',
            zh: '剩余游玩时间',
        },
    },
    achievements: {
        title: {
            ru: 'Мои достижения',
            ua: 'Мої досягнення',
            en: 'My Achievements',
            de: 'Meine Erfolge',
            pl: 'Moje osiągnięcia',
            zh: '我的成就',
        },
        subtitle: {
            ru: 'Делай больше, получай больше!',
            ua: 'Роби більше, отримуй більше!',
            en: 'Do more, get more!',
            de: 'Mach mehr, bekomm mehr!',
            pl: 'Rób więcej, zyskuj więcej!',
            zh: '做得越多，得到越多！',
        },

        counterLabel: {
            ru: 'Достижений',
            ua: 'Досягнень',
            en: 'Achievements',
            de: 'Erfolge',
            pl: 'Osiągnięć',
            zh: '成就',
        },
        reward: {
            ru: 'Награда',
            ua: 'Нагорода',
            en: 'Reward',
            de: 'Belohnung',
            pl: 'Nagroda',
            zh: '奖励',
        },
        claim: {
            ru: 'Получить награду',
            ua: 'Отримати нагороду',
            en: 'Claim reward',
            de: 'Belohnung abholen',
            pl: 'Odbierz nagrodę',
            zh: '领取奖励',
        },
        claimed: {
            ru: 'Получено',
            ua: 'Отримано',
            en: 'Claimed',
            de: 'Erhalten',
            pl: 'Odebrano',
            zh: '已领取',
        },
        tabs: {
            all: {
                ru: 'Все достижения',
                ua: 'Усі досягнення',
                en: 'All achievements',
                de: 'Alle Erfolge',
                pl: 'Wszystkie osiągnięcia',
                zh: '所有成就',
            },
            incomplete: {
                ru: 'Невыполненные достижения',
                ua: 'Невиконані досягнення',
                en: 'Incomplete achievements',
                de: 'Unerfüllte Erfolge',
                pl: 'Nieukończone osiągnięcia',
                zh: '未完成成就',
            },
            completed: {
                ru: 'Выполненные достижения',
                ua: 'Виконані досягнення',
                en: 'Completed achievements',
                de: 'Erfüllte Erfolge',
                pl: 'Ukończone osiągnięcia',
                zh: '已完成成就',
            },
            top: {
                ru: 'Топ игроков',
                ua: 'Топ гравців',
                en: 'Top players',
                de: 'Top-Spieler',
                pl: 'Najlepsi gracze',
                zh: '玩家排行',
            },
        },
        empty: {
            ru: 'Здесь пока нет достижений',
            ua: 'Тут поки немає досягнень',
            en: 'No achievements here yet',
            de: 'Hier gibt es noch keine Erfolge',
            pl: 'Brak osiągnięć',
            zh: '暂无成就',
        },
    },
    reward: {
        pcs: {
            ru: 'шт.',
            ua: 'шт.',
            en: 'pcs',
            de: 'Stk.',
            pl: 'szt.',
            zh: '个',
        },
        // Labels of the stat-style reward list (components/RewardList.vue, layout 'stats').
        money: {
            ru: 'Деньги',
            ua: 'Гроші',
            en: 'Money',
            de: 'Geld',
            pl: 'Pieniądze',
            zh: '金钱',
        },
        exp: {
            ru: 'Опыт',
            ua: 'Досвід',
            en: 'Experience',
            de: 'Erfahrung',
            pl: 'Doświadczenie',
            zh: '经验',
        },
        coin: {
            ru: 'Olymp-Coins',
            ua: 'Olymp-Coins',
            en: 'Olymp-Coins',
            de: 'Olymp-Coins',
            pl: 'Olymp-Coins',
            zh: 'Olymp-Coins',
        },
        chips: {
            ru: 'Фишки',
            ua: 'Фішки',
            en: 'Chips',
            de: 'Chips',
            pl: 'Żetony',
            zh: '筹码',
        },
    },
    inventory,
    settings,
    macros,
};
