export const server = {
    success: {
        ua: 'Успішно',
        en: 'Success',
        ru: 'Успешно',
        de: 'Erfolgreich',
        pl: 'Sukces',
        zh: '成功',
    },
    error: {
        ua: 'Помилка',
        en: 'Error',
        ru: 'Ошибка',
        de: 'Fehler',
        pl: 'Błąd',
        zh: '错误',
    },
    info: {
        ua: 'Інформація',
        en: 'Info',
        ru: 'Информация',
        de: 'Info',
        pl: 'Informacja',
        zh: '信息',
    },
    warning: {
        ru: 'Внимание',
        ua: 'Увага',
        en: 'Warning',
        de: 'Achtung',
        pl: 'Uwaga',
        zh: '警告',
    },
    serverRestartHeader: {
        ru: 'ВНИМАНИЕ',
        ua: 'УВАГА',
        en: 'WARNING',
        de: 'ACHTUNG',
        pl: 'UWAGA',
        zh: '警告',
    },
    serverRestartAnnouncementChat: {
        ru: '~r~ВНИМАНИЕ! Уважаемые игроки, через {{minutes}} минут будет рестарт сервера!',
        ua: '~r~УВАГА! Шановні гравці, через {{minutes}} хвилин буде рестарт сервера!',
        en: '~r~WARNING! Dear players, the server will restart in {{minutes}} minutes!',
        de: '~r~ACHTUNG! Liebe Spieler, der Server wird in {{minutes}} Minuten neu gestartet!',
        pl: '~r~UWAGA! Drodzy gracze, za {{minutes}} minut nastąpi restart serwera!',
        zh: '~r~警告！各位玩家，服务器将在 {{minutes}} 分钟后重启！',
    },
    serverRestartAnnouncementNotify: {
        ru: 'Через {{minutes}} минут будет рестарт сервера!',
        ua: 'Через {{minutes}} хвилин буде рестарт сервера!',
        en: 'The server will restart in {{minutes}} minutes!',
        de: 'Der Server wird in {{minutes}} Minuten neu gestartet!',
        pl: 'Za {{minutes}} minut nastąpi restart serwera!',
        zh: '服务器将在 {{minutes}} 分钟后重启！',
    },
    serverRestartCanceledChat: {
        ru: '~r~ВНИМАНИЕ! Уважаемые игроки, рестарт сервера отменен!',
        ua: '~r~УВАГА! Шановні гравці, рестарт сервера скасовано!',
        en: '~r~WARNING! Dear players, the server restart has been canceled!',
        de: '~r~ACHTUNG! Liebe Spieler, der Serverneustart wurde abgesagt!',
        pl: '~r~UWAGA! Drodzy gracze, restart serwera został anulowany!',
        zh: '~r~警告！各位玩家，服务器重启已取消！',
    },
    serverRestartCanceledNotify: {
        ru: 'Рестарт сервера отменен!',
        ua: 'Рестарт сервера скасовано!',
        en: 'The server restart has been canceled!',
        de: 'Der Serverneustart wurde abgesagt!',
        pl: 'Restart serwera został anulowany!',
        zh: '服务器重启已取消！',
    },
    male: {
        ua: 'Чоловіча',
        en: 'Male',
        ru: 'Мужской',
        de: 'Männlich',
        pl: 'Mężczyzna',
        zh: '男性',
    },
    female: {
        ua: 'Жіноча',
        en: 'Female',
        ru: 'Женский',
        de: 'Weiblich',
        pl: 'Kobieta',
        zh: '女性',
    },
    pcs: {
        ru: 'шт.',
        ua: 'шт.',
        en: 'pcs',
        de: 'Stk.',
        pl: 'szt.',
        zh: '片数',
    },
    money: {
        ru: 'Деньги',
        ua: 'Гроші',
        en: 'Money',
        de: 'Geld',
        pl: 'Pieniądze',
        zh: '钱',
    },
    notEnoughMoney: {
        ua: 'У вас недостатньо коштів',
        en: 'You do not have enough money',
        ru: 'У вас недостаточно средств',
        de: 'Du hast nicht genügend Geld',
        pl: 'Nie masz wystarczająco dużo pieniędzy',
        zh: '你没有足够的钱',
    },
    negativeBalance: {
        ru: 'У вас отрицательный баланс',
        en: 'You have a negative balance.',
        ua: "У вас від'ємний баланс.",
        de: 'Du hast einen negativen Kontostand.',
        pl: 'Masz ujemne saldo.',
        zh: '你的结余是负的',
    },
    inDev1: {
        ru: 'В разработке',
        ua: 'У розробці',
        en: 'In Development',
        de: 'In Entwicklung',
        pl: 'W trakcie rozwoju',
        zh: '研制中',
    },
    inDev2: {
        ru: 'Данная функция пока находится в разработке ;)',
        ua: 'Ця функція поки знаходиться в розробці ;)',
        en: 'This feature is currently in development ;)',
        de: 'Diese Funktion befindet sich derzeit in Entwicklung ;)',
        pl: 'Ta funkcja jest obecnie w trakcie rozwoju ;)',
        zh: '此功能目前正在开发中',
    },
    lowLevel: {
        ua: 'Для цієї роботи ваш рівень повинен бути {{level}}',
        en: 'For this job, your level should be {{level}}',
        ru: 'Для этой работы ваш уровень должен быть {{level}}',
        de: 'Für diese Aufgabe sollte dein Level {{level}} sein',
        pl: 'Dla tego zadania twój poziom powinien wynosić {{level}}',
        zh: '对于这项工作，你的等级应该是{{level}}',
    },
    strangerMale: {
        ru: 'Незнакомец',
        ua: 'Незнайомець',
        en: 'Stranger',
        de: 'Fremder',
        pl: 'Nieznajomy',
        zh: '陌生人',
    },
    strangerFemale: {
        ru: 'Незнакомка',
        ua: 'Незнайомка',
        en: 'Stranger (female)',
        de: 'Fremde',
        pl: 'Nieznajoma',
        zh: '陌生人(女性)',
    },
    voiceHasBeenReloaded: {
        ru: 'Войс-чат был перезагружён',
        ua: 'Голосовий чат був перезавантажений',
        en: 'Voice chat has been reloaded',
        de: 'Voice-Chat wurde neu geladen',
        pl: 'Czat głosowy został ponownie załadowany',
        zh: '语音聊天已重新加载',
    },
    mustBeInVehicle: {
        ru: 'Вы должны находиться в транспортном средстве',
        ua: 'Ви повинні знаходитися в транспортному засобі',
        en: 'You must be in a vehicle',
        de: 'Du mmusst dich in einem Fahrzeug befinden',
        pl: 'Musisz być w pojeździe',
        zh: '你一定是在车里',
    },
    sms: {
        ru: '[СМС]',
        ua: '[СМС]',
        en: '[SMS]',
        de: '[SMS]',
        pl: '[SMS]',
        zh: '[短信]',
    },

    noSpaceInInventory: {
        ru: 'Недостаточно места в инвентаре',
        ua: 'Недостатньо місця в інвентарі',
        en: 'Not enough space in inventory',
        de: 'Nicht genug Platz im Inventar',
        pl: 'Za mało miejsca w ekwipunku',
        zh: '库存空间不足',
    },

    // DELAY
    waitDelay: {
        ru: 'Остановись!',
        ua: 'Зачекай!',
        en: 'Hold up!',
        de: 'Warte mal!',
        pl: 'Poczekaj!',
        zh: '支撑!',
    },

    messageDelay: {
        ru: 'Не так быстро ковбой!',
        ua: 'Не так швидко, ковбой!',
        en: 'Not so fast cowboy!',
        de: 'Mach langsamer!',
        pl: 'Nie tak szybko kowboju!',
        zh: '别急，牛仔!',
    },

    // Player too far
    playerTooFar: {
        ru: 'Игрок слишком далеко от вас',
        ua: 'Гравець занадто далеко від вас',
        en: 'The player is too far from you',
        de: 'Der Spieler ist zu weit von dir entfernt',
        pl: 'Gracz jest zbyt daleko od ciebie',
        zh: '玩家离你太远了',
    },

    // Are u gay?
    areUGay: {
        ru: 'Это действие можно сделать только с игроком противоположного пола',
        ua: 'Цю дію можна виконати лише з гравцем протилежної статі',
        en: 'This action can only be performed with a player of the opposite gender',
        de: 'Diese Aktion kann nur mit einem Spieler des anderen Geschlechts durchgeführt werden',
        pl: 'Tę czynność można wykonać tylko z graczem przeciwnej płci',
        zh: '这个动作只能由异性玩家执行',
    },

    //
    playerIsTooFarAway: {
        ru: 'Игрок слишком далеко для этого действия',
        ua: 'Гравець занадто далеко для цієї дії',
        en: 'The player is too far away for this action',
        de: 'Der Spieler ist zu weit weg für diese Aktion',
        pl: 'Gracz jest za daleko, żeby wykonać tę akcję',
    },
    vehicleIsTooFarAway: {
        ru: 'Машина слишком далеко для этого действия',
        ua: 'Машина занадто далеко для цієї дії',
        en: 'The vehicle is too far away for this action',
        de: 'Das Fahrzeug ist zu weit weg für diese Aktion',
        pl: 'Pojazd jest za daleko, żeby wykonać tę akcję',
    },

    //
    cantSellRentedVehicle: {
        ru: 'Ну попытка не пытка, продать а рендованную машину нельзя',
        ua: 'Ну, спробував - і досить, але орендовану машину не продати',
        en: "Well, worth a try, but you can't sell a rented car",
        de: 'Na ja, ein Versuch war’s wert, aber du kannst kein gemietetes Auto verkaufen',
        pl: 'No cóż, próba nie zaszkodzi, ale wynajętego auta nie sprzedasz',
    },

    noRecipes: {
        ru: 'У вас нет изученных чертежей для крафтов',
        ua: 'У вас немає вивчених креслень для крафтів',
        en: 'You have no learned blueprints for crafting',
        de: 'Du hast keine erlernten Baupläne fürs Crafting',
        pl: 'Nie masz poznanych schematów do craftingu',
    },
    skillLevelUp: {
        ru: 'Вы достигли нового уровня навыка {{skill}}.',
        ua: 'Ви досягли нового рівня навику {{skill}}.',
        en: 'You reached a new level of the {{skill}} skill.',
        de: 'Du hast ein neues Level der Fertigkeit {{skill}} erreicht.',
        pl: 'Osiągnięto nowy poziom umiejętności {{skill}}.',
        zh: '你已达到技能 {{skill}} 的新等级。',
    },

    skillNames: {
        stamina: {
            ru: 'Выносливость',
            ua: 'Витривалість',
            en: 'Stamina',
            de: 'Ausdauer',
            pl: 'Wytrzymałość',
            zh: '耐力',
        },
        strength: {
            ru: 'Сила',
            ua: 'Сила',
            en: 'Strength',
            de: 'Kraft',
            pl: 'Siła',
            zh: '力量',
        },
        driving_ability: {
            ru: 'Мастерство вождения',
            ua: 'Майстерність водіння',
            en: 'Driving skill',
            de: 'Fahrkönnen',
            pl: 'Umiejętność jazdy',
            zh: '驾驶能力',
        },
        lung_capacity: {
            ru: 'Дыхание',
            ua: 'Дихання',
            en: 'Lung capacity',
            de: 'Lungenkapazität',
            pl: 'Pojemność płuc',
            zh: '肺活量',
        },
        wheelie_ability: {
            ru: 'Баланс мотоцикла (вилли)',
            ua: 'Баланс мотоцикла (віллі)',
            en: 'Wheelie control',
            de: 'Wheelie-Kontrolle',
            pl: 'Kontrola wheelie',
            zh: '机车抬头控制',
        },
        flying_ability: {
            ru: 'Лётное мастерство',
            ua: 'Льотна майстерність',
            en: 'Flying ability',
            de: 'Flugfähigkeit',
            pl: 'Umiejętność latania',
            zh: '飞行能力',
        },
        shooting_ability: {
            ru: 'Стрельба',
            ua: 'Стрільба',
            en: 'Shooting',
            de: 'Schießen',
            pl: 'Strzelanie',
            zh: '射击',
        },
        stealth_ability: {
            ru: 'Скрытность',
            ua: 'Скритність',
            en: 'Stealth',
            de: 'Schleichen',
            pl: 'Skradanie',
            zh: '潜行',
        },
    },
};
