export const server = {
    action: {
        ru: 'Для взаимодействия нажмите',
        ua: 'Для взаємодії натисніть',
        en: 'Press to interact',
        de: 'Drücke um zu interagieren',
        pl: 'Naciśnij, aby interakcji',
        zh: '按下进行交互',
    },
    productShop: {
        ru: 'Чтобы открыть магазин 24/7',
        ua: 'Щоб відкрити магазин 24/7',
        en: 'To open the 24/7 store',
        de: 'Um den 24/7 Laden zu öffnen',
        pl: 'Aby otworzyć sklep 24/7',
        zh: '要全天候开店',
    },
    weaponShop: {
        ru: 'Чтобы открыть каталог магазина оружия',
        ua: 'Щоб відкрити каталог магазину зброї',
        en: 'To open the weapon shop catalog',
        de: 'Um den Waffenladenkatalog zu öffnen',
        pl: 'Aby otworzyć katalog sklepu z bronią',
        zh: '打开武器商店目录',
    },
    clothesShop: {
        ru: 'Чтобы открыть каталог магазина одежды',
        ua: 'Щоб відкрити каталог магазину одягу',
        en: 'To open the clothing store catalog',
        de: 'Um den Kleidungsladenkatalog zu öffnen',
        pl: 'Aby otworzyć katalog sklepu odzieżowego',
        zh: '打开服装店目录',
    },
    pickupItem: {
        ru: 'Чтобы подобрать предмет, нажмите',
        ua: 'Щоб підібрати предмет, натисніть',
        en: 'To pick up the item, press',
        de: 'Um den Gegenstand aufzuheben, drücke',
        pl: 'Aby podnieść przedmiot, naciśnij',
        zh: '要拾取项目，请按',
    },
    truckerMenu: {
        ru: 'Чтобы открыть меню дальнобойщика',
        ua: 'Щоб відкрити меню дальнобойщика',
        en: 'To open the trucker menu',
        de: 'Um das Trucker-Menü zu öffnen',
        pl: 'Aby otworzyć menu truckera',
        zh: '打开卡车司机菜单',
    },
    deliveryMenu: {
        ru: 'Чтобы открыть меню доставки еды',
        ua: 'Щоб відкрити меню доставки їжі',
        en: 'To open the food delivery menu',
        de: 'Um das Menü für die Lieferung von Essen zu öffnen',
        pl: 'Aby otworzyć menu dostawy jedzenia',
        zh: '打开送餐菜单',
    },
    quarryMenu: {
        ru: 'Чтобы открыть меню работы на карьере',
        ua: "Щоб відкрити меню роботи на кар'єрі",
        en: 'To open the quarry work menu',
        de: 'Um das Menü für die Arbeit im Steinbruch zu öffnen',
        pl: 'Aby otworzyć menu pracy w kamieniołomie',
    },
    job: {
        ru: 'Чтобы открыть меню устройства на работу нажмите',
        ua: 'Щоб відкрити меню пристрою на роботу, натисніть',
        en: 'To open the job device menu, press',
        de: 'Um das Arbeitsgerätemenü zu öffnen, drücken Sie',
        pl: 'Aby otworzyć menu urządzenia na pracę, naciśnij',
        zh: '要打开作业设备菜单，请按',
    },
    rent: {
        ua: 'Щоб відкрити меню оренди',
        en: 'To open the rental menu',
        ru: 'Чтобы открыть меню аренды',
        de: 'Um das Mietmenü zu öffnen',
        pl: 'Aby otworzyć menu wynajmu',
        zh: '打开租赁菜单',
    },
    carshop: {
        ua: 'Щоб відкрити меню автосалону',
        en: 'To open the car shop menu',
        ru: 'Чтобы открыть меню автосалона',
        de: 'Um das Menü des Autohauses zu öffnen',
        pl: 'Aby otworzyć menu salonu samochodowego',
        zh: '打开汽车旅馆的菜单',
    },
    lsc: {
        ua: 'Щоб відкрити меню тюнінг-ательє',
        en: 'To open the tuning workshop menu',
        ru: 'Чтобы открыть меню тюнинг-ателье',
        de: 'Um das Menü der Tuning-Werkstatt zu öffnen',
        pl: 'Aby otworzyć menu warsztatu tuningowego',
        zh: '打开调音艺术菜单',
    },
    atm: {
        ua: 'Щоб відкрити меню банкомату',
        en: 'To open the ATM menu',
        ru: 'Чтобы открыть меню банкомата',
        de: 'Um das ATM-Menü zu öffnen',
        pl: 'Aby otworzyć menu bankomatu',
        zh: '打开ATM菜单',
    },
    bank: {
        ua: 'Щоб відкрити меню банка',
        en: 'To open the bank menu',
        ru: 'Чтобы открыть меню банка',
        de: 'Um das Bank-Menü zu öffnen',
        pl: 'Aby otworzyć menu banku',
        zh: '打开银行菜单',
    },
    arrest: {
        ru: 'Оформление',
        ua: 'Оформлення',
        en: 'Processing',
        de: 'Bearbeitung',
        pl: 'Przetwarzanie',
        zh: '处理',
    },
    toTalk: {
        ru: 'Чтобы поговорить нажмите',
        ua: 'Щоб поговорити натисніть',
        en: 'To talk, click',
        de: 'Um zu sprechen, drücke',
        pl: 'Aby porozmawiać, kliknij',
        zh: '要发言，请单击',
    },
    houseInventory: {
        ru: 'Чтобы открыть инвентарь дома нажмите',
        ua: 'Щоб відкрити інвентар дома, натисніть',
        en: 'To open the house inventory, press',
        de: 'Um das Hausinventar zu öffnen, drücke',
        pl: 'Aby otworzyc magazyn domu, naciśnij',
        zh: '要打开房子，请按',
    },
    gymBench: {
        ru: 'Чтобы начать тренировку на жиме лёжа, нажмите',
        ua: 'Щоб почати тренування на жимі лежачи, натисніть',
        en: 'To start bench press training, press',
        de: 'Um das Bankdrücken zu starten, drücke',
        pl: 'Aby rozpocząć wyciskanie na ławce, naciśnij',
        zh: '要开始卧推训练，请按',
    },
    gymChinups: {
        ru: 'Чтобы начать тренировку на турнике, нажмите',
        ua: 'Щоб почати тренування на турніку, натисніть',
        en: 'To start chin-ups training, press',
        de: 'Um das Klimmzugtraining zu starten, drücke',
        pl: 'Aby rozpocząć trening podciągania, naciśnij',
        zh: '要开始引体向上训练，请按',
    },
    gymSitups: {
        ru: 'Чтобы начать тренировку на пресс, нажмите',
        ua: 'Щоб почати тренування на прес, натисніть',
        en: 'To start sit-ups training, press',
        de: 'Um das Bauchtraining zu starten, drücke',
        pl: 'Aby rozpocząć trening brzuszków, naciśnij',
        zh: '要开始仰卧起坐训练，请按',
    },
    gymMuscle: {
        ru: 'Чтобы начать тренировку со штангой, нажмите',
        ua: 'Щоб почати тренування зі штангою, натисніть',
        en: 'To start barbell training, press',
        de: 'Um das Langhanteltraining zu starten, drücke',
        pl: 'Aby rozpocząć trening ze sztangą, naciśnij',
        zh: '要开始杠铃训练，请按',
    },
    gymOnehand: {
        ru: 'Чтобы начать тренировку с гантелями, нажмите',
        ua: 'Щоб почати тренування з гантелями, натисніть',
        en: 'To start dumbbell training, press',
        de: 'Um das Kurzhanteltraining zu starten, drücke',
        pl: 'Aby rozpocząć trening z hantlami, naciśnij',
        zh: '要开始哑铃训练，请按',
    },
};
