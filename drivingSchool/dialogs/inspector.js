export default {
    messages: {
        busy: {
            ru: 'Я занята, если вам что-то нужно обратитесь к нашему менеджеру возле входа.',
            ua: 'Я зайнята, якщо тобі щось потрібно - звернись до нашого менеджера біля входу.',
            en: "I'm busy. If you need anything, please speak to our manager near the entrance.",
            de: 'Ich bin beschäftigt. Wenn du etwas brauchst, wende dich bitte an unseren Manager am Eingang.',
            pl: 'Jestem zajęta. Jeśli czegoś potrzebujesz, zgłoś się do naszego menedżera przy wejściu.',
        },

        receiveLicense: {
            ru: 'Поздравляю вас с успешным сдачей экзамена на категорию {{category}}! Вот ваши права. Не забудьте продлевать их каждые 60 дней.\n\nУдачи на дорогах!',
            ua: 'Вітаю з успішною здачею іспиту на категорію {{category}}! Ось ваше посвідчення. Не забудьте продовжувати його кожні 60 днів.\n\nУдачі на дорогах!',
            en: 'Congratulations on passing the exam for category {{category}}! Here is your license. Don’t forget to renew it every 60 days.\n\nGood luck on the road!',
            de: 'Herzlichen Glückwunsch zum Bestehen der Prüfung für die Kategorie {{category}}! Hier ist dein Führerschein. Vergiss nicht, ihn alle 60 Tage zu verlängern.\n\nViel Glück auf den Straßen!',
            pl: 'Gratulacje z okazji zdania egzaminu na kategorię {{category}}! Oto twoje prawo jazdy. Nie zapomnij odnawiać go co 60 dni.\n\nPowodzenia na drodze!',
        },

        renewLicense: {
            ru: 'Поздравляю вас с успешным сдачей экзамена на категорию {{category}}! Срок их действия продлён на 60 дней.\n\nВозвращайтесь к нам снова!',
            ua: 'Вітаю з успішною здачею іспиту на категорію {{category}}! Термін дії продовжено на 60 днів.\n\nПовертайтеся до нас знову!',
            en: 'Congratulations on passing the exam for category {{category}}! The validity has been extended by 60 days.\n\nCome back to us again!',
            de: 'Herzlichen Glückwunsch zum Bestehen der Prüfung für die Kategorie {{category}}! Die Gültigkeit wurde um 60 Tage verlängert.\n\nKomm gerne wieder zu uns!',
            pl: 'Gratulacje z okazji zdania egzaminu na kategorię {{category}}! Ważność została przedłużona o 60 dni.\n\nZapraszamy ponownie!',
        },

        renewLicenseForMoney: {
            ru: 'Оплата принята, срок действия ваших прав категории {{category}} продлён на 60 дней.\n\nТолько тщщ! Никому не говорите что у нас доступны такие услуги.',
            ua: 'Оплата прийнята, термін дії ваших прав категорії {{category}} продовжено на 60 днів.\n\nТільки тс-с-с! Нікому не кажіть, що у нас доступні такі послуги.',
            en: "Payment accepted, your {{category}} license has been extended for 60 days.\n\nJust shhh! Don't tell anyone that we offer such services.",
            de: 'Zahlung angenommen, dein Führerschein der Kategorie {{category}} wurde um 60 Tage verlängert.\n\nNur pssst! Erzählen Sie niemandem, dass wir solche Dienstleistungen anbieten.',
            pl: 'Płatność przyjęta, ważność Twojego prawa jazdy kategorii {{category}} została przedłużona o 60 dni.\n\nTylko ciii! Nikomu nie mów, że oferujemy takie usługi.',
        },
    },
    buttons: {
        okay: {
            ru: 'Ладно, извините...',
            ua: 'Гаразд, вибачте...',
            en: 'Alright, sorry...',
            de: 'Okay, Entschuldigung...',
            pl: 'Dobrze, przepraszam...',
        },
        thanks: {
            ru: 'Спасибо!',
            ua: 'Дякую!',
            en: 'Thanks!',
            de: 'Danke!',
            pl: 'Dzięki!',
        },
    },
};
